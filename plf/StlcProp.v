...
Proof.
  ...
  - right. inversion Ht; subst; clear Ht.
    destruct (IHt1 _ H2).
    + destruct (IHt2 _ H4).
      * eapply canonical_forms_fun in H2; [| assumption].
        destruct H2 as [x [t0 H1]]. subst.
        exists (<{ [x:=t2]t0 }>). auto.
      * destruct H0 as [t2']. exists <{t1 t2'}>. auto.
    + destruct H as [t1']. exists <{t1' t2}>. auto.
  - inversion Ht; subst. right.
    destruct (IHt1 _ H3) as [|[t1']].
    + destruct (canonical_forms_bool t1); subst; eauto.
...
Proof.
  ...
  generalize dependent Gamma.
  induction Ht; intros Gamma' G; simpl; eauto; subst.
  - unfold update, t_update in H.
    destruct (eqb_spec x x0); [|auto]. subst.
    inversion H; subst.
    apply weakening_empty. assumption.
...
Proof.
   ...
   *)
   ...
   repeat split; eauto.
   intros Ht. inversion Ht; subst; clear Ht.
...
Proof.
  ...
  induction Hmulti.
  - destruct (progress _ _ Hhas_type); congruence.
...
Proof.
  ...
  induction e; intros; inversion H; inversion H0; subst; eauto.
  - congruence.
  - pose proof (IHe2 _ T2 T3 H6 H12); subst.
    pose proof (IHe1 Gamma <{{T3->T}}> <{{T3->T'}}>).
    injection H1; auto.
  - f_equal. eauto.
Qed.
(** [] *)
...
Proof.
  ...
  inversion H1; subst.
  destruct (IHappears_free_in _ _ H7) as [TT].
  exists TT. rewrite update_neq in H2; auto.
Qed.
(** [] *)
...
Proof.
  unfold closed.
  intros. intros Hfree.
  destruct (free_in_context _ _ _ _ Hfree H).
  inversion H0.
Qed.
(** [] *)
...
Theorem progress0 : forall (t:tm) (T:ty),
  <{empty |-- t \in T}> ->
  value t \/ exists t', t --> t'.
Proof. exact progress. Qed.

Theorem preservation0 : forall t t' T,
  <{empty |-- t \in T}> ->
  t --> t' ->
  <{empty |-- t' \in T}>.
...
Inductive step : tm -> tm -> Prop :=
  | SA_App1 : forall t1 t1' t2, t1 --> t1' -> <{t1 t2}> --> <{t1' t2}>
  | SA_App2 : forall v1 t2 t2', t2 --> t2' -> value v1 -> <{v1 t2}> --> <{v1 t2'}>
  | SA_AppAbs : forall x T t v, value v -> <{(\x:T,t)v}> --> <{[x:=v]t}>
  | SA_Succ1 : forall t t', t --> t' -> <{succ t}> --> <{succ t'}>
  | SA_Succ2 : forall n : nat, <{succ n}> --> S n
  | SA_Pred1 : forall t t', t --> t' -> <{pred t}> --> <{pred t'}>
  | SA_Pred2 : forall n : nat, <{pred n}> --> (Nat.pred n)
  | SA_Mult1 : forall t1 t1' t2, t1 --> t1' -> <{t1*t2}> --> <{t1'*t2}>
  | SA_Mult2 : forall v1 t2 t2', t2 --> t2' -> value v1 -> <{v1*t2}> --> <{v1*t2'}>
  | SA_MultConstConst : forall n1 n2:nat, <{n1*n2}> --> Nat.mul n1 n2
  | SA_If0 : forall t1 t1' t2 t3, t1 --> t1' -> <{if0 t1 then t2 else t3}> --> <{if0 t1' then t2 else t3}>
  | SA_If0True : forall t1 t2, <{if0 0 then t1 else t2}> --> t1
  | SA_If0False : forall n t1 t2, <{if0 $(S n) then t1 else t2}> --> t1
where "t '-->' t'" := (step t t').
...
Inductive has_type : context -> tm -> ty -> Prop :=
  | T_Const : forall (n:nat) c, <{c|--n\in Nat}>
  | T_Succ : forall c t, <{c|--t\in Nat}> -> <{c|--succ t \in Nat}>
  | T_Pred : forall c t, <{c|--t\in Nat}> -> <{c|--pred t \in Nat}>
  | T_Mult : forall c t1 t2, <{c|--t1\in Nat}> -> <{c|--t2\in Nat}> -> <{c|--t1 * t2 \in Nat}>
  | T_Var : forall c (x:string) T, c x = Some T -> <{c|--x\in T}>
  | T_If0 : forall c t1 t2 t3 T, <{c|--t1\in Nat}> -> <{c|--t2\in T}> -> <{c|--t3\in T}> ->
      <{c|--if0 t1 then t2 else t3 \in T}>
  | T_App : forall c t1 t2 T T2, <{c|--t1\in T2 -> T}> -> <{c|--t2\in T2}> -> <{c|--t1 t2\in T}>
  | T_Abs : forall c x t T1 T2, <{x |-> T1;c|--t\in T2}> -> <{c|--\x:T1,t\in T1->T2}>
...
Proof.
  ...
  induction H0; intros; eauto using includedin_update.
Qed.
...
Lemma weakening_empty : forall Gamma t T,
      ...
      <{ Gamma |-- t \in T }>.
Proof.
  intros. apply weakening with empty; auto.
  unfold includedin. intros. inversion H0.
Qed.

Lemma substitution_preserves_typing : forall Gamma x s t T S,
  <{ x |-> S; Gamma |-- t \in T}> ->
  <{ empty |-- s \in S }> ->
  <{ Gamma |-- [x:=s]t \in T}>.
Proof.
  ...
  - unfold update, t_update in H3.
    destruct (eqb_spec x s0); subst; auto.
    inversion H3; subst. apply weakening_empty. auto.
  - destruct (eqb_spec x s0).
    + subst. apply T_Abs. rewrite update_shadow in H6. auto.
    + apply T_Abs. apply IHt with S; auto.
      rewrite update_permute; auto.
Qed.
...
Proof with eauto.
  ...
  induction H0; intros; try (inversion H; eauto; fail); try (inversion H1)...
  - inversion H0; subst.
    inversion H4; subst.
...
Proof.
  intros.
  inversion H0; subst; [eauto | inversion H].
Qed.
...
Proof with eauto.
  ...
  remember empty as Gamma.
  induction H; subst...
  - right. destruct IHhas_type as [|[t']]...
    destruct canonical_forms_nat with t; subst...
  - right. destruct IHhas_type as [|[t']]...
    destruct canonical_forms_nat with t; subst...
  - destruct IHhas_type1 as [|[t']]...
    destruct canonical_forms_nat with t1; subst...
    destruct IHhas_type2 as [|[t']]...
    destruct canonical_forms_nat with t2; subst...
  - inversion H.
  - destruct IHhas_type1 as [|[t']]...
    destruct canonical_forms_nat with t1; subst...
    destruct x0; eauto.
  - right. destruct IHhas_type1 as [|[t']]...
    inversion H1; subst.
    * inversion H.
    * destruct IHhas_type2 as [|[t']]...
Qed.
