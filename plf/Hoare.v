...
Proof.
  ...
  pose proof (contra empty_st (X!->1) H I).
...
Theorem hoare_asgn_fwd :
  ...
  {{ $(fun st => (P (X !-> m ; st)
             /\ st X = aeval (X !-> m ; st) a))  }}.
Proof.
  intros m a P st st' H1 [H2 H3].
  inversion H1. subst. simpl in H3.
  assert ((X !-> m; X !-> aeval st a; st) = st).
  { rewrite t_update_shadow. rewrite <- H3. apply t_update_same. }
  rewrite H.
  split; auto.
Qed.
(** [] *)
...
Theorem hoare_asgn_fwd_exists :
  ...
  {{ $(fun st => exists m, P (X !-> m ; st) /\
                st X = aeval (X !-> m ; st) a) }}.
Proof.
  intros a P st st' H1 H2.
  inversion H1. subst.
  exists (st X).
  assert ((X !-> st X; X !-> aeval st a; st) = st).
  { rewrite t_update_shadow. apply t_update_same. }
  rewrite H.
  split; auto.
Qed.

(* My question: Are these 3 [hoare_asgn]s same, or,
   can they describe hoare triple of asignment sentence completely? *)
...
Example hoare_asgn_example4 :
  ...
  {{ X = 1 /\ Y = 2 }}.
Proof.
  eapply hoare_seq with (Q := {{ X = 1 }}); simpl.
  - eapply hoare_consequence_pre.
    + apply hoare_asgn.
    + eauto.
  - eapply hoare_consequence_pre.
...
Definition swap_program : com := <{ X:=X+Y; Y:=X-Y; X:=X-Y }>.
...
Proof.
  ...
  intros H.
  simpl in H.
  specialize H with (a := AId X) (n := 0) (st := empty_st)
    (st' := (Y!->3;X!->3)).
  assert (3 = 0 -> False) by lia.
  apply H0. apply H.
...
Proof.
  intros P Q b c H1 H2 st st' HE HP.
  inversion HE; subst; eauto.
Qed.
...
Lemma hoare_if1_good :
  ...
  {{ X = Z }}.
Proof.
  eapply hoare_if1; simpl; eapply hoare_consequence_pre;
    try apply hoare_asgn;
    try apply hoare_skip; assertion_auto'.
  - destruct H.
    destruct (st Y =? 0) eqn:E; simpl in *; try congruence.
    assertion_auto'.
Qed.
...
Proof.
  intros P Q b c Hc st st' Hre Hst.
  inversion Hre; subst. apply Hc in H2. apply H2 in Hst.
  clear Hre H2 st.
  remember <{ while ~ b do c end }> as prog.
  induction H4; inversion Heqprog; subst; simpl in *.
  - rewrite negb_false_iff in H. rewrite H in Hst.
    destruct Hst as [[contra] | [_ H1]]; try congruence. auto.
  - rewrite negb_true_iff in H. rewrite H in Hst.
    destruct Hst as [[_ H1] | [contra]]; try congruence.
    apply IHceval2; eauto.
Qed.
...
Theorem hoare_repeat_example :
  ...
  {{ X = 0 /\ Y > 0 }}.
Proof.
  assert ( hoare_repeat_example_helper:
    {{X > 0}} Y := X; X := X - 1 {{~ <{ X = 0 }> /\ X > 0 \/ <{ X = 0 }> /\ Y > 0}}
  ). {
    simpl.
    intros st st' H1 H2.
    inversion H1; inversion H4; inversion H6; subst.
    unfold t_update. simpl.
    rewrite eqb_eq.
    lia.
  }
  simpl in *.
  (* 用 post 处理 eqb_eq 更简单? *)
  intros st st' H1 H2.
  rewrite <- eqb_eq.
  pose proof hoare_repeat ({{ X > 0}}) ({{ Y > 0 }}) (<{X=0}>) (<{Y:=X;X:=X-1}>) hoare_repeat_example_helper.
  apply (H st st'); auto.
Qed.
...
Proof.
  ...
  - intros contra.
    ...
    specialize contra with empty_st RError.
    assert ((exists st, RError = RNormal st /\ True) -> False). {
      intros. destruct H as [st [H _]]. inversion H.
    }
...
(* FILL IN HERE *)
Theorem hoare_assert : forall P (b:bexp), {{P /\ b}} assert b {{P /\ b}}.
Proof.
  unfold valid_hoare_triple.
  intros.
  inversion H; subst.
  - eauto.
  - destruct H0 as [_ H0]. congruence.
Qed.

Theorem hoare_assume : forall P (b:bexp), {{P}} assume b {{P /\ b}}.
Proof.
  unfold valid_hoare_triple.
  intros.
  inversion H. subst.
...
(*
  We show that this [hoare_assume] is the strongest Hoare rules for [assume].
  [hoare_assume] is complete.
  recall sp_property:
  forall P b Q, {{P}} assume b {{Q}} -> sp P <{assume b}> -> Q.
*)

Theorem hoare_assume_complete : forall P b Q st,
  {{P}} assume b {{Q}} -> ({{P /\ b}}) st -> Q st.
Proof.
  intros. simpl in *. destruct H0 as [H0 H1].
  unfold valid_hoare_triple in H.
  assert (st =[ assume b ]=> RNormal st). {
    apply E_Assume. auto.
  }
  specialize H with st (RNormal st).
  pose proof H H2 H0.
  destruct H3 as [st' [H4 H5]].
  inversion H4. auto.
Qed.
...
Proof.
  ...
  - eapply hoare_seq.
    + eapply hoare_consequence_post.
      * apply hoare_assert with (P:={{True}}).
      * auto.
    + apply hoare_asgn.
  - eapply hoare_consequence_post.
    + apply hoare_assume.
