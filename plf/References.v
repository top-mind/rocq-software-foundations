...
(* Yes, by the small step semantic of [let]. *)
...
Proof with unfold tseq; auto; simpl.
  exists <{ (ref 0 := (\ x : Nat, ! (loc 1) x)); (ref 1 := (\ x : Nat, ! (loc 0) x)) }>.
  eapply multi_step...
  eapply multi_step...
  eapply multi_step...
  eapply multi_step...
...
Theorem preservation_wrong1_wrong : exists ST T t st t' st',
  <{ empty / ST |-- t \in T }> /\
  t /st --> t'/st' /\
  ~ <{ empty / ST |-- t' \in T }>.
Proof with auto.
  exists nil, <{{Ref Nat}}>, <{ref 0}>, nil, <{loc 0}>, (tm_const 0 :: nil).
  split...
  split...
...
Theorem store_not_unique:
  exists st, exists ST1, exists ST2,
    ...
    ST1 <> ST2.
Proof with auto.
  exists (<{!(loc 0)}> :: nil), (<{{Unit}}> :: nil), (<{{Nat}}> :: nil).
  split.
  - unfold store_well_typed. split...
    simpl. intros. replace l with 0 by lia.
    unfold store_lookup.
...
Definition factorial : tm :=
  <{(\r:Ref(Nat -> Nat), \y:Nat,
    (r:=(\y:Nat,
      if0 y then 1 else (((!r) (pred y)) * y)));
    (!r y))
    ref (\y : Nat, 1)
  }>.

(* XXX note that tseq is buggy, it shadows outer x: *)
Theorem type_bug : <{ empty/nil |-- \x:Nat,((ref 0:=0);x) \in Nat->Unit}>.
Proof with eauto.
  apply T_Abs.
  eapply T_App...
Qed.
(* I may use pair in [MoreStlc.v] t1 ; t2 := (t1, t2).snd as an alternative
   grammar sugar *)
