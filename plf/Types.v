...
Proof.
  intros.
  destruct H; unfold step_normal_form; intros contra; destruct contra.
  - inversion H; subst; inversion H0.
  - generalize dependent x.
    induction t; intros; inversion H0; inversion H.
    eauto.
  (* induction H; intros.
    + inversion H0.
    + inversion H0; subst.
      eapply IHnvalue. apply H2. *)
Qed.
...
Theorem step_deterministic:
  deterministic step.
Proof with eauto.
  assert (suc_v__P: forall P v t, nvalue v -> <{succ v}> --> t -> P). {
    intros. exfalso. assert (value <{succ v}>). { right... }
    apply value_is_nf in H1. apply H1...
  }
  unfold deterministic.
  intros. generalize dependent y2.
  induction H; intros; inversion H0; subst; try solve_by_invert;
    try (apply IHstep in H5; rewrite H5);
    try (apply IHstep in H2; rewrite H2);
    try reflexivity;
    try (eapply suc_v__P; try apply H; apply H2).
Qed.
(** [] *)
...
Proof with eauto.
  ...
  induction H0; intros; inversion H; auto; inversion H0; auto.
  subst. apply succ_hastype_nat__hastype_nat. assumption.
Qed.
(** [] *)
...
Proof.
  ...
  ~ <{ |-- if false then 0 else true \in Bool }>.
  *)
  specialize contra with (t:=<{if false then 0 else true}>) (t':=<{true}>) (T:=Bool).
  apply has_type_not. apply contra.
  split; auto.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard (variation2)
   ...
   this rule?  For each one that does, give a counter-example.
    - Determinisim of [step]. <{ if true then true else false }> --> true
                                                                 --> false
*)
(* Do not modify the following line: *)
...
(** **** Exercise: 2 stars, standard, optional (variation3)
   ...
   Which of the above properties become false in the presence of
   this rule?  For each one that does, give a counter-example.
   - Deterministic. <{ if true then (pred 0) else 0 }> --> <{pred 0}>
