...
Proof with eauto.
  ...
  - invert_typecheck Gamma t0 T1.
  - fully_invert_typecheck Gamma t1 T0 T01 T02.
    rename s into x1.
    rename s0 into x2.
    remember <{ x1|-> T01; Gamma }> as Gamma1.
    invert_typecheck Gamma1 t2 T1.
    remember <{ x2|-> T02; Gamma }> as Gamma2.
    invert_typecheck Gamma2 t3 T2.
    case_equality T1 T2.
  (* lists (the [tm_lcase] is given for free) *)
  - constructor.
  - invert_typecheck Gamma t1 T1.
    fully_invert_typecheck Gamma t2 T2 Tl T_.
    case_equality T1 Tl.
  - (* tlcase *)
  ...
  - invert_typecheck Gamma t1 T1.
    ...
    invert_typecheck Gamma' t2 T2.
  (* fix *)
  - fully_invert_typecheck Gamma t Tt T1 T2.
    case_equality T1 T2.
Qed.
(** [] *)
...
Fixpoint stepf (t : tm) : option tm :=
  ...
  | <{ inl T2 t1 }> =>
    match stepf t1 with
    | Some t1' => Some <{ inl T2 t1' }>
    | None => None
    end
  | <{ inr T1 t2 }> =>
    match stepf t2 with
    | Some t2' => Some <{ inr T1 t2' }>
    | None => None
    end
  | <{ case t0 of | inl x1 => t1 | inr x2 => t2 }> =>
    match stepf t0, t0 with
    | Some t0', _ => Some <{ case t0' of | inl x1 => t1 | inr x2 => t2 }>
    | None, <{ inl T2 v1 }> => assert (valuef v1) (Some <{ [x1:=v1]t1 }>)
    | None, <{ inr T1 v2 }> => assert (valuef v2) (Some <{ [x2:=v2]t2 }>)
    | _, _ => None
    end
  (* lists (the [tm_lcase] is given for free) *)
  | <{ nil T }> => None
  | <{ t1 :: t2 }> => match stepf t1, stepf t2 with
  ...
  | <{t.fst}> => match stepf t, t with
    | Some t', _ => Some <{t'.fst}>
    | None, <{(v1, v2)}> => assert (valuef v1) (assert (valuef v2) (Some v1))
    | _, _ => None
    end
  | <{t.snd}> => match stepf t, t with
    | Some t', _ => Some <{t'.snd}>
    | None, <{(v1, v2)}> => assert (valuef v1) (assert (valuef v2) (Some v2))
    | _, _ => None
    end
  (* let *)
  | <{let x=t1 in t2}> => match stepf t1 with
    | Some t1' => Some <{let x=t1' in t2}>
    | None => assert (valuef t1) (Some <{[x:=t1]t2}>)
    end
  (* fix *)
  | <{fix t}> => match stepf t, t with
    | Some t', _ => Some <{fix t'}>
    | _, <{\x:T1, t1}> => Some <{[x:=fix (\x:T1, t1)]t1}>
    | _, _ => None
    end
  end.
(* Do not modify the following line: *)
...
Proof.
  intros.
  induction t; try discriminate; auto.
...
Proof.
  intros.
  apply complete_valuef in H.
  induction t; simpl in H; try discriminate; auto.
  - simpl. rewrite (IHt H). auto.
  - simpl. rewrite (IHt H). auto.
  - rewrite andb_true_iff in H. destruct H as [H1 H2].
    simpl. rewrite (IHt1 H1). rewrite (IHt2 H2). auto.
  - rewrite andb_true_iff in H. destruct H as [H1 H2].
    simpl. rewrite (IHt1 H1). rewrite (IHt2 H2). auto.
Qed.
...
Theorem complete_stepf : forall t t',
    t --> t'  ->  stepf t = Some t'.
Proof with auto.
  Tactic Notation "val" ident(H) := rewrite (value_stepf_nf _ H);
    rewrite (complete_valuef _ H); auto.
  intros t.
  induction t; intros; try solve_by_invert;
  inversion H; subst; simpl;
  try rewrite (IHt _ H1);
  try rewrite (IHt _ H3);
  try rewrite (IHt1 _ H3);
  try rewrite (IHt1 _ H4);
  try rewrite (IHt1 _ H6);
  try rewrite (IHt2 _ H4); auto;
  try val H1; try val H2; try val H3; try val H4; try val H6; try val H7.
Qed.
