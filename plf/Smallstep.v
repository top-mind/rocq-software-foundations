...
Proof.
  ...
  - inversion H0; subst; try solve_by_invert.
    + f_equal. auto.
    + solve_by_inverts 2.
  - inversion H; inversion H1; subst; try solve_by_invert.
    f_equal. auto.
Qed.
...
Proof.
  ...
  - unfold normal_form. intros H. destruct H.
...
Proof.
  ...
  - destruct IHt1.
    ...
    + destruct H. right. exists (test x t2 t3). apply ST_If. auto.
Qed.
(** [] *)
...
Proof.
  unfold deterministic.
  intros. generalize dependent y2.
  induction H; intros; inversion H0; subst; try solve_by_invert; try reflexivity.
...
Example bool_step_prop4_holds :
  bool_step_prop4.
Proof. apply ST_ShortCircuit. Qed.
(** [] *)
...
Proof.
  eapply multi_step.
  - apply ST_Plus2; try apply v_const.
    apply ST_Plus2; try apply v_const.
    apply ST_PlusConstConst.
  - apply multi_R.
    apply ST_Plus2; try apply v_const.
...
Proof.
  ...
  rewrite nf_same_as_value in *.
  generalize dependent y2.
  induction P11; intros; inversion P12; inversion P22; subst.
  - inversion P21; subst; try reflexivity. inversion H.
  - apply IHP11; auto.
    inversion P21; subst.
    + inversion H.
    + assert (y = y0). {
...
Proof.
  ...
  - apply multi_trans with (P (C n1) t2).
    ...
    + apply multi_trans with (P (C n1) (C n2)).
      * apply multistep_congr_2; auto. apply v_const.
      * apply multi_R. apply ST_PlusConstConst.
Qed.
(** [] *)
...
Proof.
  ...
  - inversion H; subst. apply E_Plus; auto.
  - inversion H0; subst. apply E_Plus; auto.
Qed.
(** [] *)
...
Proof.
  ...
  intros.  destruct H.
  rewrite nf_same_as_value in H0. inversion H0; subst.
...
Proof.
  ...
  - inversion H0; subst; try solve_by_invert; try solve_by_inverts 2.
    f_equal. auto.
  - inversion H1; subst; try solve_by_invert; try solve_by_inverts 2.
    f_equal. auto.
  - inversion H0; subst; try solve_by_invert; try reflexivity.
  - inversion H0; subst; try solve_by_invert; try reflexivity.
  - inversion H0; subst; try solve_by_invert; try reflexivity.
...
Proof.
  right. intros contra.
  specialize contra with (test (C 0) tru tru).
...
Proof.
  assert (stack_append : forall st e1 e2 s s' s'',
    stack_multistep st (e1, s) ([], s') ->
    stack_multistep st (e1 ++ e2, s ++ s'') (e2, s' ++ s'')). {
    induction e1; intros.
    - inversion H; subst.
      + apply multi_refl.
      + inversion H0.
    - destruct a; inversion H; inversion H0; subst;
      eapply IHe1 in H1; eapply multi_step; try constructor; apply H1.
  }
  specialize stack_append with (s:=[]).
  unfold compiler_is_correct_statement.
  intros e.
  induction e; intros; simpl; try (eapply multi_R; constructor);
  eapply multi_trans; try (apply stack_append; apply IHe1);
  eapply multi_trans; try (rewrite app_nil_r; apply stack_append; apply IHe2);
