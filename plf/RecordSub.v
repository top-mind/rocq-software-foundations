...
Inductive subtype : ty -> ty -> Prop :=
  ...
  | S_RcdDepth : forall i S1 T1 Sr2 Tr2,
    S1 <: T1 ->
    Sr2 <: Tr2 ->
    record_ty Sr2 -> (* subtype guarantees well type *)
    record_ty Tr2 ->
    <{{ i : S1 :: Sr2 }}> <: <{{ i : T1 :: Tr2 }}>
...
Proof with eauto.
  ...
  apply S_Trans with <{{j:B->B::k:A->A::nil}}>.
  - apply S_RcdPerm...
    discriminate.
  - apply S_RcdDepth...
Qed.
...
Proof with eauto.
  ...
  - apply S_Trans with <{{ y : B :: z : C :: x : A :: nil}}>.
    + apply S_RcdDepth... apply S_RcdPerm... discriminate.
    + apply S_RcdPerm... discriminate.
Qed.
(** [] *)
...
Proof with (eauto using wf_rcd_lookup).
  ...
  - (* S_RcdPerm *)
    ...
    + (* subtype *)
      ...
      apply S_Refl.
      eapply wf_rcd_lookup; [|apply Hget]...  Qed.
...
Proof with eauto.
  ...
  induction Hs; intros; subst; try solve_by_invert.
  - (* S_Refl *)
    exists V1, V2.
    inversion H. subst...
  - (* S_Trans *)
    destruct (IHHs2 V1 V2) as [U1 [U2 [? [? ?]]]]...
    destruct (IHHs1 U1 U2) as [S1 [S2 [? [? ?]]]]...
    exists S1, S2...
  - (* S_Arrow *)
    inversion HeqV; subst. eexists...
Qed.
(** [] *)
...
Proof with eauto.
  unfold TRcd_j.
  eapply T_App; [|apply typing_example_0].
  eapply T_Sub.
  - apply T_Abs...
    eapply T_Proj...
    unfold Tlookup. simpl...
  - unfold TRcd_kj.
    unfold TRcd_j.
    apply S_Arrow...
    apply subtyping_example_1.
...
Proof with eauto.
  ...
  - apply T_Abs; unfold TRcd_j...
    eapply T_Proj.
    + eapply T_App...
      apply T_Var...
    + auto.
  - apply T_Abs. auto.
    eapply T_Sub; [| apply subtyping_example_1].
