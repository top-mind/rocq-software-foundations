...
Proof.
  ...
  - eapply H_Consequence_pre.
    ...
    + intros. apply H.
  - econstructor; auto.
  - eapply H_Consequence with (P':={{True}}); auto.
    + apply H_While. auto.
Qed. 
...
Proof.
  ...
  - eapply H_Consequence_pre.
    ...
    + intros. destruct H.
  - econstructor; auto.
  - econstructor; eapply H_Consequence_pre;
    try apply IHc1; try apply IHc2; intros;
    destruct H as [[]].
...
Proof.
  Ltac auto' := unfold valid in *; eauto.
  intros P c. generalize dependent P.
  induction c; intros.
  - remember <{skip}> as p.
    induction X; inversion Heqp; subst.
    apply hoare_skip. eauto. auto'.
  - remember <{ x := a }> as p.
    induction X; inversion Heqp; subst.
    apply hoare_asgn. auto'.
  - remember <{ c1; c2 }> as p.
    induction X; inversion Heqp; subst.
    + eapply hoare_seq. apply IHc2. eauto.
      apply IHc1. auto.
    + auto'.
  - remember <{ if b then c1 else c2 end }> as p.
    induction X; inversion Heqp; subst.
    + apply hoare_if. apply IHc1. auto. apply IHc2. auto.
    + auto'.
  - remember <{ while b do c end}> as p.
    induction X; inversion Heqp; subst.
    + apply hoare_while. apply IHc. auto.
    + auto'.
Qed.
...
Proof.
  ...
  - eapply H_If.
    ...
    + apply IHc2. intros. destruct H0. simpl in H1.
      destruct (beval st b) eqn:Eb; try congruence.
      eauto.
  - eapply H_Consequence.
    + apply H_While. apply IHc. apply wp_invariant.
    + unfold wp. intros. eapply HT. apply H0. auto.
    + unfold wp. simpl. intros. destruct H.
      destruct (beval st b) eqn:Eb; try congruence.
      eauto.
Qed.
