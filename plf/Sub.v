...
(** **** Exercise: 2 stars, standard, especially useful (arrow_sub_wrong)
    ...
    Sol: f=\s:Student,s.gpa and g=\f:(P->N),f {name=J,age=10}.
    g f -->* {name=J,age=10}.gpa. Stuck!
*)
...
(** **** Exercise: 2 stars, standard (small_large_1)
   ...
   - What is the _largest_ type [T] that makes the same assertion true? A->A
...
(** **** Exercise: 2 stars, standard (small_large_2)
   - What is the _smallest_ type [T] that makes the following
     assertion true?
       ...
       empty |-- (\p:(A->A * B->B), p) ((\z:A,z), (\z:B,z)) \in T (A->A*B->B)
    TODO formal proof

   - What is the _largest_ type [T] that makes the same assertion true? Top
...
(** **** Exercise: 2 stars, standard, optional (small_large_3)
   - What is the _smallest_ type [T] that makes the following
     assertion true?
       ...
       a:A |-- (\p:(A*T), (p.snd) (p.fst)) (a, \z:A,z) \in A (A->A)
...
(** **** Exercise: 2 stars, standard (small_large_4)
   - What is the _smallest_ type [T] (if one exists) that makes the
     following assertion true?
       ...
       exists S,
         empty |-- (\p:(A*T), (p.snd) (p.fst)) \in S does not exists
...
(** **** Exercise: 2 stars, standard (smallest_1)
    ...
    the following assertion true? Does not exist
...
(** **** Exercise: 3 stars, standard, optional (count_supertypes)
    ...
    18 {x:A,y:C->C}*6*2 {x:A}*2 {y:C->C} * 3 Top*1
    [] *)
...
(** **** Exercise: 2 stars, standard (pair_permutation)
    ...
    No. Consider (\x:A*B,x.fst). It has type A obviously. If adding permutation
    rule, {x:A*B |-- x \in B*A }. {empty |-- \x:A*B,x.fst \in B}. Then it can have
    different base type.
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_Sub : forall Gamma t1 T1 T2,
      ...
      T1 <: T2 ->
      <{ Gamma |-- t1 \in T2 }>
  | T_Pair : forall T1 T2 Gamma t1 t2,
      <{ Gamma |-- t1 \in T1 }> ->
      <{ Gamma |-- t2 \in T2 }> ->
      <{ Gamma |-- (t1, t2) \in T1 * T2 }>
  | T_Fst: forall T1 T2 Gamma t0,
      <{ Gamma |-- t0 \in T1*T2 }> ->
      <{ Gamma |-- t0.fst \in T1}>
  | T_Snd: forall T1 T2 Gamma t0,
      <{ Gamma |-- t0 \in T1*T2 }> ->
      <{ Gamma |-- t0.snd \in T2}>
...
Theorem typing_example_0 : <{empty |-- ((\z:A,z), (\z:B,z)) \in ((A->A) * (B->B))}>.
Proof.
  apply T_Pair; auto.
Qed.
...
(* empty |-- (\x:Top * (B->B), x.snd) ((\z:A,z), (\z:B,z))
         \in B->B *)
Theorem typing_example_1 : <{ empty |-- (\x:Top * (B->B), x.snd) ((\z:A,z), (\z:B,z)) \in B->B}>.
Proof with eauto.
  eapply T_App.
  - apply T_Abs...
  - apply T_Pair...
...
(* empty |-- (\z:(C->C)->(Top * B->B), (z (\x:C,x)).snd)
              (\z:C->C, ((\z:A,z), (\z:B,z)))
         \in B->B *)
Theorem typing_example_2 : <{ empty |-- (\z:(C->C)->(Top * (B->B)), (z (\x:C,x)).snd)
...
Proof with eauto.
  ...
  induction Hs; intros; try discriminate...
  - rename V1 into T1. rename V2 into T2.
    destruct (IHHs2 _ _ HeqV) as [U1 [U2 [HeqU []]]].
    destruct (IHHs1 _ _ HeqU) as [S1 [S2 [HeqS []]]].
    exists S1, S2...
...
Proof with auto.
  intros U Hs.
  remember <{{ Top }}> as V.
  induction Hs; try discriminate...
  destruct IHHs1...
Qed.
(** [] *)

Lemma sub_inversion_Pair : forall S T1 T2,
    S <: <{{ T1 * T2 }}> ->
    exists S1 S2, S = <{{ S1 * S2 }}> /\ S1 <: T1 /\ S2 <: T2.
Proof with eauto.
  intros.
  remember <{{T1 * T2}}> as T.
  ...
  - edestruct IHsubtype2 as [S1 [S2 [Hty [Hs1 Hs2]]]]...
    destruct (IHsubtype1 _ _ Hty) as [U1 [U2 [Hty2 [Hu1 Hu2]]]].
    exists U1, U2...
...
Lemma canonical_forms_of_arrow_types : forall Gamma s T1 T2,
  ...
  exists x S1 s2,
     s = <{\x:S1,s2}>.
Proof with eauto.
  intros Gamma s T1 T2 H.
  remember <{{T1 -> T2}}> as TArrow.
  generalize dependent T2.
  generalize dependent T1.
  induction H; intros; subst; try solve_by_invert...
  destruct (sub_inversion_arrow _ _ _ H0) as [S1 [S2 []]].
  eapply IHhas_type...
Qed.
(** [] *)
...
Lemma canonical_forms_of_Pair : forall Gamma T1 T2 t,
  <{ Gamma |-- t \in T1 * T2 }> ->
  value t ->
  exists t1 t2, t = <{(t1, t2)}> /\ <{ Gamma |-- t1 \in T1 }> /\ <{ Gamma |-- t2 \in T2 }>.
Proof with eauto.
  intros.
  remember <{{ T1 * T2 }}> as T0.
  generalize dependent T2.
  generalize dependent T1.
  induction H; intros; subst; try solve_by_invert...
  - apply sub_inversion_Pair in H1.
    destruct H1 as [S1 [S2 [Hty [Hs1 Hs2]]]].
    destruct (IHhas_type H0 _ _ Hty) as [U1 [U2 [Hty2 [Hu1 Hu2]]]].
    exists U1, U2...
  ...
  - (* T_Pair *)
    ...
    + destruct IHHt2; subst...
      * destruct H0 as [t2' Hstep].
        right. exists <{ (t1, t2') }>...
    + destruct H as [t1']. right. exists <{ (t1', t2) }>...
  - (* T_Fst *)
    destruct IHHt; auto; right.
    + apply canonical_forms_of_Pair in Ht...
      destruct Ht as [t1 [t2 [Heq ]]]. subst.
      inversion H; subst...
    + destruct H. eexists...
  - (* T_Snd *)
    destruct IHHt; auto; right.
...
Proof with eauto.
  intros Gamma x T H.
  (* we need to remember the term, not the string *)
  remember (tm_var x) as x0.
  induction H; try solve_by_invert.
  - (* although we have Heqx0 : x0 = x, but that is actually tm_var x0 = tm_var x *)
    inversion Heqx0; subst...
  - inversion Heqx0; subst.
    destruct IHhas_type as [S [Hcontext Hty]]...
Qed.
...
Proof with eauto.
  ...
  destruct IHhas_type as [S2 [Ht1 Ht2]]...
Qed.
(** [] *)
...
Proof with eauto.
  ...
  - apply sub_inversion_Pair in H0.
    destruct H0 as [S1 [S2 [Hty [Hs1 Hs2]]]].
    edestruct IHhas_type...
  - inversion Heqt0; subst. inversion HeqT0; subst...
Qed.
...
Proof.
  ...
  - destruct (eqb_spec x x0); subst.
    + rewrite update_shadow in Ht. eauto.
    + apply T_Abs. apply IHHt. apply update_permute. assumption.
Qed.
...
Proof with eauto.
  ...
  - inversion HE; subst...
    apply typing_inversion_pair in HT. destruct HT...
  - inversion HE; subst...
    apply typing_inversion_pair in HT. destruct HT...
Qed.
...
Theorem unit_inversion : forall x, ~<{ empty |-- \x:Top,x \in Unit }>.
Proof with eauto.
  intros x H.
  remember empty as Gamma.
  remember <{\x:Top,x}> as t.
  remember <{{Unit}}> as T.
...
(** **** Exercise: 2 stars, standard (variations)
    ...
    - Suppose we add the following reduction rule:
                             ...
                             --------------------         (ST_Funny21)
                             unit --> (\x:Top. x)
      preservation: unit \in Unit, (\x:Top,x) not in Unit (unit_inversion)

    - Suppose we add the following subtyping rule:

                               ----------------          (S_Funny3)
                               Unit <: Top->Top
      progress: unit unit \in Top but can not progress
    ...
    - Suppose we add the following reduction rule:
                             ...
                             (unit t) --> (t unit)
      neither
      (for preservation, appending proof:
      + inversion HT1.
        apply typing_inversion_unit in HT1.
        apply sub_inversion_arrow in HT1.
        destruct HT1 as [U1 [U2 [Hcontra _]]]. discriminate.)
    ...
    - Suppose we add the same reduction rule _and_ a new typing rule:
      ...
      preservation: unit true \in Top, unit true --> true unit but the latter has no type
    ...
    - Suppose we _change_ the arrow subtyping rule to:
                          ...
                          -----------------              (S_Arrow')
                          S1->S2 <: T1->T2
      preservation: (\x:Bool,x) unit --> unit
      Theorem type_bool : forall x, <{ empty |-- (\x:Bool,x) unit \in Bool }>.
      Proof with eauto.
        intros.
        eapply T_App with <{{Top}}>...
      Qed.
      Theorem type_ill : ~<{empty |-- unit \in Bool}>.
      Proof.
        intros H.
        remember <{unit}> as tu.
        remember <{{Bool}}> as Tb.
        induction H; try discriminate.
        subst. apply sub_inversion_Bool in H0. auto.
      Qed.
      (for progress, modify the conclusion in sub_inversion_arrow.)
...
Proof with eauto.
  right. intros H.
  remember <{{Unit}}> as u.
  assert (<{{ (Top->Top)->u }}> <: <{{ (u->u) -> u}}>)...
  apply sub_inversion_arrow in H0.
  destruct H0 as [U1 [U2 [H1 [H2 H3]]]]. subst.
  apply sub_inversion_Unit in H3. subst.
  inversion H1; subst. clear H1.
  apply sub_inversion_arrow in H2.
  destruct H2 as [U1 [U2 [H1 [H0 H3]]]]. inversion H1. subst.
  apply sub_inversion_Unit in H0. discriminate.
Qed.
(** [] *)
...
Theorem formal_subtype_instances_tf_1g:
  TF (forall S T U V, S <: T -> U <: V ->
         <{{ S*V }}> <: <{{ T*U }}>).
Proof with auto.
  right. intros H. pose proof (H Ty_Top Ty_Top Ty_Bool Ty_Top).
  assert (<{{ Top * Top }}> <: <{{ Top * Bool }}>) by auto.
  apply sub_inversion_Pair in H1.
  destruct H1 as [S1 [S2 [Heq [_ Hcontra]]]].
  inversion Heq; subst.
  apply sub_inversion_Bool in Hcontra.
  discriminate.
Qed.
(** [] *)
...
Proof.
  right.
  intros H.
  specialize H with Ty_Bool Ty_Top.
  assert (<{{ Bool -> Bool }}> <: <{{ Top -> Top }}>) by auto.
  apply sub_inversion_arrow in H0.
  destruct H0 as [U1 [U2 [Heq [Hcontra]]]].
  inversion Heq; subst.
  apply sub_inversion_Bool in Hcontra. discriminate.
Qed.
(** [] *)
...
Proof.
  right.
  intros H. 
  specialize H with <{{Top -> A}}>.
  assert (<{{ Top -> A }}> <: <{{ A -> A }}>) by auto.
  destruct (H H0) as [T [H1 H2]].
...
Proof with eauto.
  right.
  intros H. destruct H as [T].
  induction T; apply sub_inversion_arrow in H;
    destruct H as [U1 [U2 [H1 [H2 H3]]]]; inversion H1; subst...
Qed.
(** [] *)
...
Proof with auto.
  right. intros H. destruct H as [T].
  assert (T <: <{{Bool}}>)...
  assert (T <: <{{Unit}}>)...
  apply sub_inversion_Bool in H0.
...
Proof.
  right. intros H. destruct H as [T1 [T2]].
  assert (forall S1 : ty, T1 <: S1). {
    intros.
    specialize H with S1 Ty_Top.
    eapply sub_inversion_Pair in H.
    destruct H as [T1' [T2' [H1 [H2]]]].
    inversion H1. auto.
  }
  pose proof (H0 Ty_Bool).
  apply sub_inversion_Bool in H1.
  pose proof (H0 Ty_Unit).
...
Proof.
  right. intros H. destruct H as [T1 [T2 ]].
  assert (forall S1,T1<:S1). {
    intros. pose proof (H S1 T2).
    apply sub_inversion_arrow in H0.
    destruct H0 as [U1 [U2 [H1 [H2 H3]]]].
    inversion H1; subst. auto.
  }
  clear H.
  pose proof (H0 A) as H1. pose proof (H0 B) as H2.
  apply sub_inversion_Base in H1. apply sub_inversion_Base in H2.
  subst. discriminate.
Qed.
(** [] *)
...
Proof.
  right. intros H. destruct H as [T1 [T2 ]].
  assert (forall S2,T2<:S2). {
    intros. pose proof (H T1 S2).
    apply sub_inversion_arrow in H0.
    destruct H0 as [U1 [U2 [H1 [H2 H3]]]].
    inversion H1; subst. auto.
  }
...
Theorem formal_subtype_concepts_tfh:
  TF (exists f : nat -> ty,
         ...
         (forall i, f i <: f (S i))).
Proof with eauto.
  left. exists (fun n:nat => <{{$(sub_dec n)->Top}}>).
  split.
  - intros i j H1 H2. apply H1. clear H1.
    generalize dependent j.
    induction i; intros.
    + destruct j... inversion H2.
    + destruct j.
      * inversion H2.
      * injection H2 as H2. f_equal. apply IHi.
        f_equal...
  - intros i. induction i...
    simpl in *. apply S_Arrow...
    apply S_Prod...
    apply sub_inversion_arrow in IHi.
    destruct IHi as [U1 [U2 [H1 [H2]]]].
    inversion H1; subst...
Qed.
(** [] *)
...
Theorem formal_proper_subtypes:
  TF (forall T,
         ...
         exists S,
           S <: T /\ S <> T).
Proof with eauto.
  (* I guess the author forget to consider pair? *)
  right. intros H. specialize H with <{{Unit * Unit}}>.
  destruct H as [S [H1 H2]].
  - intros H. destruct H as [|[[x]|]]; inversion H.
  - apply H2. apply sub_inversion_Pair in H1. destruct H1 as [S1 [S2 [H3 [H4 H5]]]].
    apply sub_inversion_Unit in H4,H5. subst...
Qed.
(** [] *)

Theorem subtype_antisymmetric : forall T1 T2,
    T1 <: T2 -> T2 <: T1 -> T1 = T2.
Proof with eauto.
  intros T1 T2 H.
  induction H; intros...
