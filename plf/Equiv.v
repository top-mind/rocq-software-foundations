...
Proof.
  split; intros.
  - destruct (beval st b) eqn:Hb; inversion H; subst;
    rewrite Hb in H5; try discriminate.
    + apply E_IfFalse; try assumption.
      simpl. rewrite Hb. reflexivity.
    + apply E_IfTrue; try assumption.
      simpl. rewrite Hb. auto.
  - destruct (beval st b) eqn:Hb; inversion H; subst;
    simpl in H5; rewrite Hb in H5; try discriminate.
...
Proof.
  ...
  - eapply while_true_nonterm; try apply H. apply H0.
  - eapply while_true_nonterm with (b:=BTrue).
...
Theorem assign_aequiv : forall (X : string) (a : aexp),
  ...
  cequiv <{ skip }> <{ X := a }>.
Proof.
  unfold aequiv, cequiv. simpl.
  split; intros; inversion H0; subst; clear H0.
  - erewrite <- t_update_same.
...
Proof.
  ...
  { unfold cequiv. intros.
    ...
    apply E_Seq with st'; try apply H; try apply H0; assumption. }
  split; apply A; auto using sym_cequiv.
Qed.
(** [] *)
...
Proof.
  assert (A: forall b b' c1 c1' c2 c2' st st',
    ...
    st =[ if b' then c1' else c2' end ]=> st').
  { unfold bequiv, cequiv.
    intros. remember (<{ if b then c1 else c2 end }>) as cif.
  ...
  split;
  apply A; auto using sym_aequiv, sym_bequiv, sym_cequiv.
Qed.
(** [] *)
...
(* FILL IN HERE *)
Definition not_congr (p1 p2 : com) : Prop := True.
Theorem sym_not_congr : forall p1 p2, not_congr p1 p2 -> not_congr p2 p1.
Proof. intros. apply I. Qed.
Theorem refl_not_congr : forall p1, not_congr p1 p1.
Proof. intros. apply I. Qed.
Theorem trans_not_congr : forall p1 p2 p3, not_congr p1 p2 -> not_congr p2 p3 ->
  not_congr p1 p3.
Proof. intros. apply I. Qed.

(* not a congruence *)
Theorem CSkip_not_congr :
  ~ (forall c st st', not_congr c CSkip -> st =[c]=> st' -> st =[skip]=> st').
Proof. intros contra.
  assert (empty_st =[ X:=1 ]=> (X!->1)). {
    apply E_Asgn. reflexivity.
  }
  pose proof (contra (<{X:=1}>) empty_st (X!->1) I H).
  inversion H0; subst. assert (empty_st X = (X !-> 1) X). {
...
Proof.
  ...
  - (* BLe *)
    ...
    remember (fold_constants_aexp a2) as a2' eqn:Heqa2'.
    replace (aeval st a1) with (aeval st a1') by
    ( symmetry; rewrite Heqa1'; apply fold_constants_aexp_sound).
    replace (aeval st a2) with (aeval st a2') by
    ( symmetry; rewrite Heqa2'; apply fold_constants_aexp_sound).
    destruct a1'; destruct a2'; try reflexivity.
    simpl. destruct (n <=? n0); reflexivity.
  - (* BGt *)
    simpl.
...
Proof.
  ...
  - (* while *)
    pose proof fold_constants_bexp_sound b.
    destruct (fold_constants_bexp b) eqn:Heqb;
...
Proof.
  ...
  induction a; try reflexivity;
    ...
    destruct a1; try destruct n; reflexivity.
Qed.
...
Proof with reflexivity.
  ...
  induction b; try reflexivity; simpl;
    (repeat rewrite <- optimize_0plus_aexp_sound); try reflexivity.
  - rewrite IHb...
  - rewrite <- IHb1; rewrite <- IHb2...
Qed.
...
Proof with reflexivity.
  ...
  induction c; try reflexivity; simpl; intros.
  - apply CAsgn_congruence. apply optimize_0plus_aexp_sound.
  - apply CSeq_congruence; unfold cequiv; intros.
    + rewrite <- IHc1... + rewrite <- IHc2...
  - apply CIf_congruence; try apply optimize_0plus_bexp_sound;
      unfold cequiv; intros.
    + rewrite IHc1...
    + rewrite IHc2...
  - apply CWhile_congruence; try apply optimize_0plus_bexp_sound;
      unfold cequiv; intros.
    rewrite IHc...
Qed.
...
Proof.
  unfold ctrans_sound, cequiv, optimizer.
  intros.
  eapply trans_cequiv.
...
Proof with try reflexivity.
  ...
  - unfold t_update.
    destruct (String.eqb_neq x y) as [_ H0].
    rewrite H0; auto.
Qed.
...
Proof with try reflexivity.
  assert (A: 
    forall st x1 a1 a2, var_not_used_in_aexp x1 a1 ->
    aeval (x1 !-> aeval st a1; st) a2 = aeval (x1 !-> aeval st a1; st) (subst_aexp x1 a1 a2)).
  { induction a2; simpl; auto; intros.
    destruct (x1 =? x)%string eqn:Ex1...
    rewrite (String.eqb_eq) in Ex1; subst.
    rewrite aeval_weakening by apply H.
    unfold t_update. rewrite String.eqb_refl... }
  intros.
  unfold cequiv. intros.
  split; intros; inversion H0; inversion H6; inversion H3; subst;
    eapply E_Seq; try apply E_Asgn;
    try rewrite <- A; try assumption...
Qed.
...
Proof.
  unfold cequiv.
  intros contra.
  pose proof (contra empty_st empty_st). clear contra.
  assert (empty_st =[ skip ]=> empty_st) by constructor.
  rewrite <- H in H0.
  eapply while_true_nonterm with (b:=<{true}>).
  - constructor.
  - apply H0.
Qed.
(** [] *)
...
Proof.
  left. unfold cequiv. split; intros.
  - inversion H; inversion H2; inversion H5; subst.
    eapply E_Seq.
    + apply E_Havoc.
    + replace (Y !-> n0; X !-> n; st) with (X !-> n; Y !-> n0; st).
      * apply E_Havoc.
      * apply t_update_permute. discriminate.
  - inversion H; inversion H2; inversion H5; subst; eapply E_Seq.
    + apply E_Havoc.
    + replace (X !-> n0; Y !-> n; st) with (Y !-> n; X !-> n0; st).
      * apply E_Havoc.
      * apply t_update_permute. discriminate.
Qed.
(** [] *)
...
Proof.
  ...
  pose proof (contra empty_st (Y!->1; X!->0)). clear contra.
  assert (empty_st =[ ptwice ]=> (Y !-> 1; X !-> 0)) by eauto using E_Seq, E_Havoc.
  rewrite H in H0.
  inversion H0; inversion H3; inversion H6; subst.
  simpl in H14. assert ((X !-> n) X = n) by reflexivity.
  rewrite H1 in H14. clear H0 H H1 H3 H6.
  assert ((Y !-> n; X !-> n) X = (Y !-> 1; X !-> 0) X) by (rewrite H14; reflexivity).
  assert ((Y !-> n; X !-> n) Y = (Y !-> 1; X !-> 0) Y) by (rewrite H14; reflexivity).
  replace ((Y !-> n; X !-> n) X) with ((Y !-> n; X !-> n) Y) in H by reflexivity.
  rewrite H in H0.
  discriminate.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl in H4. destruct (st' X =? 0) eqn:E; try discriminate.
    apply H. apply eqb_eq. apply E.
  - assert (A: forall st st', st =[ havoc Y; X := X + 1]=> st' ->
      st' X =? 0 = false).
    { intros. inversion H0. inversion H9. subst.
      unfold t_update. rewrite String.eqb_refl. simpl.
      apply eqb_neq. lia. }
    pose proof (A _ _ H3).
    clear H H2 H3 st.
    remember <{ while ~ X = 0 do havoc Y; X := X + 1 end }> as loop.
    induction H6; inversion Heqloop; subst.
    + simpl in H. rewrite H0 in H. discriminate.
    + apply IHceval2; try reflexivity. apply A with st. auto.
Qed.

Lemma p2_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p2 ]=> st'.
Proof.
  intros. intros contra. inversion contra; subst; clear contra.
  - simpl in H4. destruct (st' X =? 0) eqn:E.
    + apply H. apply eqb_eq. apply E.
    + discriminate.
  - inversion H3; subst; clear H3. clear H.
    remember <{ while ~ X = 0 do skip end }> as loop.
    induction H6; inversion Heqloop; subst.
    + rewrite H in H2. discriminate.
    + apply IHceval2; inversion H6_; subst.
...
Proof.
  ...
  - unfold p1, p2. split; intros;
    inversion H; subst;
    try (apply E_WhileFalse; apply H4); simpl in H2;
    rewrite E in H2; discriminate.
  - split; intros; rewrite eqb_neq in E.
    + pose proof (p1_may_diverge _ st' E).
      destruct (H0 H).
    + pose proof (p2_may_diverge _ st' E).
      destruct (H0 H).
Qed.
...
Proof.
  unfold cequiv. intros contra.
  remember (X!->1) as st.
  remember (Z!->0; X!->0; Z!->1; st) as st'.
  pose proof (contra st st').
  assert (st =[ p3 ]=>st').
  { eapply E_Seq.
    ...
    - apply E_WhileTrue with st'.
      ...
      + apply E_WhileFalse. subst. reflexivity. }
  rewrite H in H0.
  unfold p4 in H0.
  inversion H0; inversion H3; inversion H6; subst.
  clear H H0 H3 H6. simpl in H15.
  assert (1 = 0).
  {
    erewrite <- t_update_eq.
    rewrite <- H15.
    reflexivity.
  }
  discriminate.
...
Proof.
  ...
  split; intros.
  - remember p5 as p5_.
    induction H; inversion Heqp5_; subst.
    + simpl in H. assert ((X!->1;st) = st).
      { destruct (st X =? 1) eqn:E; try discriminate.
        rewrite eqb_eq in E. rewrite <- E.
        apply t_update_same. }
      replace (st =[ p6 ]=> st) with (st =[ p6 ]=> (X !-> 1; st)).
      * apply E_Asgn.  reflexivity.
      * rewrite H0. reflexivity.
    + simpl in H. pose proof (IHceval2 Heqp5_). clear IHceval1 IHceval2 Heqp5_ H1 H.
  ...
  - inversion H; subst; simpl; clear H.
    ...
    + replace 1 with (st X) by (apply eqb_eq; apply E).
      rewrite t_update_same.
      apply E_WhileFalse. simpl; rewrite E; reflexivity.
...
Proof.
  ...
  - inversion H2; inversion H5; inversion H8; subst.
    assert (A: (aeval (l1 !-> aeval st a1; st) a2) = (aeval st a2)). {
      apply aeval_weakening. assumption.
    } rewrite A.
    assert (B: (l2 !-> aeval st a2; l1 !-> aeval st a1; st) = 
      (l1 !-> aeval st a1; l2 !-> aeval st a2; st) ).
    { apply functional_extensionality.
      intros.
      unfold t_update.
      destruct (l2 =? x)%string eqn:E1, (l1 =? x)%string eqn:E2; try reflexivity.
      apply String.eqb_eq in E1,E2. subst. destruct H. reflexivity. }
    rewrite B.
    eapply E_Seq.
    + apply E_Asgn. reflexivity.
    + apply E_Asgn. apply aeval_weakening. auto.
  - rename l1 into tmp; rename l2 into l1; rename tmp into l2.
    rename a1 into tmp; rename a2 into a1; rename tmp into a2.
    inversion H2; inversion H5; inversion H8; subst.
    assert (A: (aeval (l1 !-> aeval st a1; st) a2) = (aeval st a2)). {
      apply aeval_weakening. assumption.
    } rewrite A.
...
Proof.
  split; unfold capprox; intro contra.
  - pose proof (contra empty_st (X!->0; empty_st)).
    assert (empty_st =[ c3 ]=> (X !-> 0)). {
      apply E_Asgn. reflexivity.
    }
    pose proof (H H0).
    ...
    discriminate.
  - pose proof (contra empty_st (X!->1; empty_st)).
    assert (empty_st =[ c4 ]=> (X !-> 1)). { apply E_Asgn. reflexivity. }
    pose proof (H H0).
    inversion H1; subst; simpl in H6.
    assert ((X !-> 0) X = (X !-> 1) X). {
      rewrite H6. reflexivity.
...
Proof.
  unfold capprox.
  intros.
  destruct (while_true_nonterm <{ true }> <{ skip }> st st').
  - apply refl_bequiv.
  - apply H.
Qed.
...
Definition zprop (c : com) : Prop :=
  exists st st', st <> st' /\ st =[ c ]=> st'.

Theorem zprop_preserving : forall c c',
  zprop c -> capprox c c' -> zprop c'.
Proof.
  unfold zprop, capprox.
  intros c c' H_zprop H_capprox.
  destruct H_zprop as [st [st' [H_neq H_eval]]].
  exists st, st'.
  split.
  - assumption.
  - apply H_capprox.
