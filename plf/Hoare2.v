...
Proof.
  verify.
  - rewrite <- leb_le. apply H0.
  - rewrite <- leb_le. verify_assertion.
  - rewrite <- H. apply parity_ge_2. auto.
  - rewrite <- H. symmetry. apply parity_lt_2. auto.
Qed.
...
<{
  {{ X = m }}
    ...
    while X <> 0 do
      {{(#fact X * Y = #fact m) /\ X <> 0 }}
      Y := Y * X {{ #fact (X - 1) * Y = #fact m }};
      X := X - 1 {{ #fact X * Y = #fact m }}
    end
  {{ Y = #fact m }}
}>.
...
Proof.
  verify;
  try rewrite andb_true_iff in H0;
  try rewrite andb_false_iff in H0; verify_assertion.
Qed.
  ...
  <{
    {{ True }}
      ...
      while X <> n do
               ...
               {{ Y+2*Z=#pow2((X+1)+1)-1/\2*Z=#pow2(X+1) }}
        Z := 2 * Z
               {{ Y+Z=#pow2((X+1)+1)-1/\Z=#pow2(X+1) }};
        Y := Y + Z
               {{ Y=#pow2((X+1)+1)-1/\Z=#pow2(X+1)}};
        X := X + 1
...
Proof with eauto.
  ...
  - simpl.
    ...
    intros.
    specialize (H st (X !-> st Y + 1; st)).
    rewrite t_update_eq in H. assert (st Y + 1 <= 5). {
      apply H...
      apply E_Asgn...
    }
    lia.
...
Proof.
  ...
  - unfold valid_hoare_triple, "->>".
    intros.
    assert (st =[X := a]=> (X !-> (a:Aexp) st; st)). {
      apply E_Asgn. auto.
    }
    pose proof (H _ _ H1 H0).
    auto.
Qed.
