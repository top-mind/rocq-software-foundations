...
Fixpoint my_nth_error {X : Type} l n : option X :=
  match l, n with
  | [], _ => None
  | a :: l', O => Some a
  | a :: l', S n => my_nth_error l' n
  end.

Theorem test_my_nth_error: forall (X : Type) (l : list X) n,
  nth_error l n = my_nth_error l n.
Proof.
...
(** **** 练习：1 星, advanced (fold_types_different) 
    ...
    [Y] 不同时的应用情景吗？ *)
Compute fold (fun a b => length a + b) [[1; 2; 3]; [4; 5]; []; [6]] 0.
(* 请在此处解答 *)
...
*)
(* 请在此处解答 *)
Theorem nth_error_none : forall X l n, length l = n -> @nth_error X l n = None.
Proof.
  induction l as [| a l' IHl'].
  - reflexivity.
  - intros n Hl'.
...
Definition plus (n m : cnat) : cnat :=
  fun (X : Type) (f : X -> X) (x : X) => n X f (m _ f x).
...
Definition exp (n m : cnat) : cnat :=
  fun (X : Type) (f : X -> X) (x : X) => m (X -> X) (fun g => n X g) f x.
...
Definition church_to_nat (c : cnat) : nat :=
  c nat (fun n => S n) 0.

Fixpoint nat_to_church (n : nat) : cnat :=
  match n with
  | O => zero
  | S n' => succ (nat_to_church n')
  end.
