...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | h :: t => if oddb h then h :: oddmembers t
              else oddmembers t
  end.
...
Theorem bag_theorem : forall b1: bag,
  true = subset b1 b1.
Proof.
  induction b1.
  - reflexivity.
  - simpl.
    replace (member n (n :: b1)) with true.
    ...
    reflexivity.
    (* true = member n (n :: b1) *)
    unfold member. (* 涉及到你尚未学过的技巧因而无法证明 *)
    simpl.
    rewrite <- eqb_refl.
    reflexivity.
Qed.
...
forall (b1 b2 : bag) (n : nat), count n b1 + count n b2 = count n (sum b1 b2).
Proof.
  intros b1 b2 n.
  induction b1 as [| h t IHb1].
  - reflexivity.
  - simpl. rewrite <- IHb1.
    destruct (h =? n).
    + reflexivity.
