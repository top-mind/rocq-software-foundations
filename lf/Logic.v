...
(** 我们已经见过很多对事实的断言（即_'命题'_）
    以及如何用证据展示其正确性（即_'证明'_）的例子了。特别是，
    我们证明了大量的_'相等性命题'_（[e1 = e2]）、蕴含式（[P -> Q]）和量化命题
    （[forall x, P x]）。在本章中，我们将会看到如何用 Coq 解决类似形式的逻辑推理。
...
(** 除了拥有类型之外，命题还是_'一等的（First-Class）'_实体，
...
(** 另一个重要的联结词是_'析取'_，即两个命题的_'逻辑或'_：若 [A] 或 [B]
    二者之一为真，则 [A \/ B] 为真。（这种中缀记法表示 [or A B]，其中
    [or : Prop -> Prop -> Prop]。） *)

(** 为了在证明中使用析取前提，我们需要分类讨论（它与 [nat]
    之类的数据类型一样，都可以显式地通过 [destruct] 或隐式地通过 [intros]
    模式来拆分： *)
...
(** ** 假命题与否定
    ...
    即证明某些给定的命题_'不是'_真的。在 Coq 中，这样的否定语句使用逻辑否定运算符
    [~] 来表达。 *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q impPQ HnQ HP.
  apply HnQ. apply impPQ. apply HP.
Qed.
(** [] *)
...
Proof.
  ...
  - induction l as [|x lt IHlt].
    + simpl. intros H. right. apply H.
    + simpl. intros [H | H].
      * left. left. apply H.
      * apply or_assoc. right. apply IHlt. apply H.
  - induction l as [|x lt IHlt].
    + simpl. intros [[] | H]. apply H.
    + simpl. intros H. rewrite <- or_assoc in H.
      destruct H as [H | H].
      * left. apply H.
...
Proof.
  ...
  - simpl. split.
    + intros H. split.
      ...
      * apply IHl'. intros x Hxl'.
        apply H. right. apply Hxl'.
    + intros [HPa HPl'] x.
      rewrite <- IHl' in HPl'.
      intros [H | H].
      * rewrite <- H. apply HPa.
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
  intros Podd Peven n H0 Hodd.
  unfold combine_odd_even in H0.
  rewrite Hodd in H0. apply H0.
Qed.
...
Theorem combine_odd_even_elim_even :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Peven n.
Proof.
  intros Podd Peven n H0 Heven.
  unfold combine_odd_even in H0.
  rewrite Heven in H0. apply H0.
Qed.
(** [] *)
...
(** 我们已经知道 [Check] 命令可以用来显式表达式的类型了，
    不过它还可以用来查找某个标识符所指代的定理。 *)

Check plus_comm : forall n m : nat, n + m = m + n.

(** 在检查定理 [plus_comm] 的_'陈述'_时，Coq 使用了与检查某项的_'类型'_一样的方式
...
Proof.
  intros n ns H.
  (* 从 A <-> B 即 A -> B /\ B -> A 提取 A -> B
    使用前面提到的 proj1 : forall A B: Prop, A /\ B -> A*)
  Compute proj1 _ _(In_map_iff _ _ _ _ _).
...
(** 目前为止我们所看见的相等关系断言基本上都只考虑了归纳类型的元素
    ...
    因此我们可以在_'任何'_类型上使用它。特别是，我们可以写出断言_'两个函数相等'_的命题： *)
...
(** **** 练习：4 星, standard (tr_rev_correct) 
    ...
    的时间复杂度与列表长度呈渐进平方关系。我们可以用以下定义来改进它： *)
...
(** 此版本的 [rev] 是_'尾递归（tail-recursive）'_的，
...
Lemma rev_append_rev :
  forall (X : Type) (l1 l2 : list X), rev_append l1 l2 = rev l1 ++ l2.
Proof.
  intros X l1.
  induction l1 as [|x l1' IHl1']; intros l2.
  - simpl. reflexivity.
  - simpl. rewrite (IHl1' (x :: l2)).
...
Proof.
  ...
  (* rev_append x [] = rev x *)
  (* 通过一般化 [] 为任意的 l2 证明: rev_append_rev *)
  rewrite (rev_append_rev _ x []). apply app_nil_r.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct IHn' as [k0].
    ...
    + exists k0. rewrite evenb_S. rewrite E. simpl. rewrite H. reflexivity.
    + exists (S k0). rewrite evenb_S. rewrite E. simpl. rewrite H. reflexivity.
Qed.
(** [] *)
...
(** 此定理说明，布尔计算 [evenb n] 的真假会从命题 [exists k, n = double k]
    的真假中_'反映（reflected）'_出来。 *)
...
(** Coq 会抱怨 [n = 2] 的类型是 [Prop]，而它想要一个 [bool]
    ...
    核心语言的_'可计算性'_特质有关，即它能表达的所有函数都是可计算且完全的。
...
Proof.
  intros b1 b2. split.
  - destruct b1.
    + split. reflexivity. apply andb_true_elim2 in H. apply H.
    + simpl. discriminate.
  - intros [H1 H2].
...
Proof.
  ...
  split.
  - intros H HH. rewrite HH in H. rewrite <- eqb_refl in H. discriminate H.
...
Proof.
  ...
  - generalize dependent l2. induction l1.
    ...
    + intros l2. destruct l2 as [|y l2'].
      * discriminate.
      * simpl. rewrite andb_true_iff. intros [E1 E2].
        apply H in E1. rewrite E1. f_equal. apply IHl1. apply E2.
  - intros E. rewrite <- E. generalize l1. intros l0.
    induction l0 as [|a l1' IHl1'].
    + reflexivity.
...
Proof.
  ...
  - induction l.
    ...
    + simpl. rewrite andb_true_iff. intros [E1 E2]. split.
      * apply E1.
      * apply IHl. apply E2.
  - induction l.
    + simpl. reflexivity.
...
(** 一般的排中律在 Coq 中默认并不可用，因为它是常见的逻辑系统（如
    ...
    当存在 [exists x, P x] 的 Coq 证明时，我们可以直接给出一个使 [P x]
...
(** 以下示例展示了为何假设排中律成立会导致非构造性证明：
    ...
    _'命题'_：存在无理数 [a] 和 [b] 使得 [a ^ b]（[a] 的 [b] 次方）为有理数。
...
(* 为了方便书写和阅读，命题标号和简写如下：
   1. peirce P
   2. double_negation_elimination DNE
   3. de_morgan DM
   4. implies_to_or ITO
   5. excluded_middle EXM
   4 -> 5 -> 2 <-> 1
     <- 3 <->
  | | 1 | 2 | 3 | 4 | 5 |
  |-| - | - | - | - | - |
  ...
  T, 3-2: proved as neccessary
  5x2: can prove by not needed
  ([1-5])[-x]([1-5]): proof relies on \1->\2
  x: hard
  dnc: do not care
  update: jacy makes by 1->2->3->4->5->1
   *)

(* 1->2 *)
Theorem P_DNE : peirce -> double_negation_elimination.
Proof.
  intros pe P HP.
  apply pe with False.
  intros HPF.
...
(* 2->1 *)
Theorem DNE_P : double_negation_elimination -> peirce.
Proof.
  intros dne P Q HPQ.
  apply dne.
  intros NP.
  assert (H: P->Q). { intros HP. apply NP in HP. destruct HP. }
  apply NP. apply HPQ. apply H.
Qed.

(* 2->3 *)
Theorem DNE_DM : double_negation_elimination -> de_morgan_not_and_not.
Proof.
  intros dne P Q HnPanQ.
  apply dne.
  intros HnPoQ.
  apply HnPanQ.
  split.
  - intros HP. apply HnPoQ. left. apply HP.
  - intros HQ. apply HnPoQ. right. apply HQ.
Qed.
...
Proof.
  ...
  assert (H: P \/ False -> P).
  { intros [H0 | []]. apply H0. }
  apply H. apply dm. intros [HPF _].
  apply HP. apply HPF.
Qed.

(* 3->4 *)
Theorem DM_ITO : de_morgan_not_and_not -> implies_to_or.
Proof.
  intros dm P Q HPQ.
  apply DM_DNE in dm as dne.
  apply dm.
  intros [HP HQ].
  apply dne in HP.
...
(* 4->5 *)
Theorem ITO_EXM : implies_to_or -> excluded_middle.
Proof.
  intros ipo P.
  apply or_commut.
  apply ipo. intros HP. apply HP.
Qed.

(* 5->2 *)
Theorem EXM_DNE : excluded_middle -> double_negation_elimination.
Proof.
  intros ex P.
  destruct (ex P).
  - intros _. apply H.
  - intros HP. apply HP in H. destruct H.
Qed.

Definition classic_logic_lemmas2 := [implies_to_or; excluded_middle].
Definition classic_logic_lemmas3 := de_morgan_not_and_not :: classic_logic_lemmas2.
Definition classic_logic_lemmas4 := double_negation_elimination :: classic_logic_lemmas3.
Definition classic_logic_lemmas5 := peirce :: classic_logic_lemmas4.

Definition classic_logic_lemmas_equal_meta := fun l => forall P Q:Prop, In P l -> In Q l -> P -> Q.

Lemma classic_logic_lemmas_equal2: classic_logic_lemmas_equal_meta classic_logic_lemmas2.
Proof.
  intros P Q.
  unfold classic_logic_lemmas2.
  intros [HP | [HP | HP]] [HQ | [HQ | HQ]].
  - rewrite <- HP. rewrite <- HQ. apply iff_refl.
  - rewrite <- HP. rewrite <- HQ. apply ITO_EXM.
  - destruct HQ.
  - rewrite <- HP. rewrite <- HQ. intros em. apply DM_ITO. apply DNE_DM. apply EXM_DNE. apply em.
  - rewrite <- HP. rewrite <- HQ. apply iff_refl.
  - destruct HQ. - destruct HP. - destruct HP. - destruct HP. 
Qed.
Lemma classic_logic_lemmas_equal3: classic_logic_lemmas_equal_meta classic_logic_lemmas3.
Proof.
  intros P Q.
  intros [HP | HP] [HQ | HQ].
  - rewrite <- HP. rewrite <- HQ. apply iff_refl.
  - rewrite <- HP. intros dm. apply classic_logic_lemmas_equal2 with (P:=implies_to_or).
    + left. reflexivity.
    + apply HQ.
    + apply DM_ITO. apply dm.
  - rewrite <- HQ. intros HP0. apply DNE_DM. apply EXM_DNE. apply ITO_EXM.
    apply classic_logic_lemmas_equal2 with (P:=P).
    + apply HP.
    + left. reflexivity.
    + apply HP0.
  - apply classic_logic_lemmas_equal2. apply HP. apply HQ.
Qed.
Lemma classic_logic_lemmas_equal4: classic_logic_lemmas_equal_meta classic_logic_lemmas4.
Proof.
  intros P Q.
  intros [HP | HP] [HQ | HQ].
  - rewrite <- HP. rewrite <- HQ. apply iff_refl.
  - rewrite <- HP. intros dne. apply classic_logic_lemmas_equal3 with (P:=de_morgan_not_and_not).
    + left. reflexivity.
    + apply HQ.
    + apply DNE_DM. apply dne.
  - rewrite <- HQ. intros HP0. apply DM_DNE.
    apply classic_logic_lemmas_equal3 with (P:=P).
    + apply HP.
    + left. reflexivity.
    + apply HP0.
  - apply classic_logic_lemmas_equal3. apply HP. apply HQ.
Qed.

Theorem classic_logic_lemmas_equal5: classic_logic_lemmas_equal_meta classic_logic_lemmas5.
Proof.
  intros P Q.
  intros [HP | HP] [HQ | HQ].
  - rewrite <- HP. rewrite <- HQ. apply iff_refl.
  - rewrite <- HP. intros p. apply classic_logic_lemmas_equal4 with (P:=double_negation_elimination).
    + left. reflexivity.
    + apply HQ.
    + apply P_DNE. apply p.
  - rewrite <- HQ. intros HP0. apply DNE_P.
    apply classic_logic_lemmas_equal4 with (P:=P).
    + apply HP.
    + left. reflexivity.
    + apply HP0.
  - apply classic_logic_lemmas_equal4. apply HP. apply HQ.
Qed.

Theorem classic_logic_lemmas_equal: forall P Q: Prop, In P classic_logic_lemmas5 -> In Q classic_logic_lemmas5 -> P <-> Q.
Proof.
  intros P Q HP HQ. split.
  apply classic_logic_lemmas_equal5. apply HP. apply HQ.
  apply classic_logic_lemmas_equal5. apply HQ. apply HP.
Qed.
