...
(** 在最常用的形式中，[;] 泛策略会接受两个策略作为参数。复合策略 [T;T']
    会先执行 [T]，然后在 [T] 生成的_'每个子目标'_中执行 [T']。 *)
...
Proof.
  intros.
  induction b; simpl;
  try (unfold optimize_0plus_b; simpl; repeat rewrite optimize_0plus_sound; reflexivity);
  [rewrite IHb |
...
(** （注意本文件顶部 [From Stdlib Require Import omega.Omega.]。）*)
...
Inductive bevalR : bexp -> bool -> Prop :=
  ...
  | E_Beq a1 a2 : BEq a1 a2 ==> (aeval a1) =? (aeval a2)
  | E_BLe a1 a2 : BLe a1 a2 ==> (aeval a1) <=? (aeval a2)
  | E_BNot b1 v1 : (b1 ==> v1) -> BNot b1 ==> (negb v1)
  | E_BAnd b1 b2 v1 v2: (b1 ==> v1) -> (b2 ==> v2) -> (BAnd b1 b2) ==> (andb v1 v2)
...
(** 现在我们可以用 [3 + (X * 2)] 来代替 [APlus 3 (AMult X 2)] 了，同样可以用
    ...
    然而这只有在我们告诉 Coq 在恰当的记法作用域中用 [%imp] 解析我们的表达式时，
    或者当 Coq 知道该表达式的类型（[aexp] 或 [bexp]）被「限定」为使用该记法作用域时才可行。 *)
...
Proof.
  ...
  induction H; intros st2 HH; inversion HH; subst;
  try reflexivity;
...
Theorem XtimesYinZ_spec : forall st x y st',
  ...
  st' Z = x * y.
Proof.
  intros. inversion H1. subst. simpl. apply t_update_eq.
Qed.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | N_Ass X n : no_whilesR (X ::= n)
  | N_Seq c1 c2 : no_whilesR c1 -> no_whilesR c2 -> no_whilesR (c1 ;; c2)
  | N_If b c1 c2 : no_whilesR c1 -> no_whilesR c2 -> no_whilesR (TEST b THEN c1 ELSE c2 FI)
.
...
Proof.
  ...
  - induction c; try constructor; inversion H;
    apply andb_true_iff in H1; destruct H1 as [H2 H3];
    try (apply IHc1; apply H2); apply IHc2; apply H3.
  - induction H; simpl;
...
Proof.
  ...
  - inversion H. subst.
    destruct (IHc1 st) as [st1]. assumption.
    destruct (IHc2 st1) as [st2]. assumption.
    exists st2.
    apply E_Seq with st1; assumption.
  - inversion H. subst.
    destruct (beval st b) eqn:E;
    [ destruct (IHc1 st) as [st'] |
      destruct (IHc2 st) as [st'] ]; try assumption;
    exists st'; [apply E_IfTrue | apply E_IfFalse]; assumption.
  - inversion H.
Qed.
(* 请勿修改下面这一行： *)
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  match prog with
  | nil => stack
  | s :: prog' => s_execute st match s with
    | SPush n => n :: stack
    | SLoad X => st X :: stack
    | _ => match stack with
      | n1 :: n2 :: stack' => match s with
        | SPlus => n2 + n1 :: stack'
        | SMinus => n2 - n1 :: stack'
        | _ => n2 * n1 :: stack'
        end
...
Theorem s_compile_app : forall st s1 p1 s2 p2 s3,
  s_execute st s1 p1 = s2 ->
  s_execute st s2 p2 = s3 ->
  s_execute st s1 (p1 ++ p2) = s3.
Proof.
  intros st s1 p1. generalize dependent s1.
  induction p1; intros.
  - subst. reflexivity.
  - apply IHp1 with s2; assumption.
Qed.

Theorem s_compile_correct' : forall (st : state) (s : list nat) (e : aexp),
  s_execute st s (s_compile e) = [ aeval st e ] ++ s.
Proof. intros. generalize dependent s.
  induction e; try reflexivity; intros; simpl; rewrite app_assoc;
    apply s_compile_app with ([aeval st e2 ; aeval st e1] ++ s);
    try apply s_compile_app with ([aeval st e1] ++ s);
    try apply IHe1;
...
(* 请在此处解答
    ...
    [] *)
Open Scope imp_scope.
Fixpoint beval_short st (b : bexp) : bool :=
  match b with
  | BTrue => true
  | BFalse => false
  | a1 = a2 => (aeval st a1) =? (aeval st a2)
  | a1 <= a2 => (aeval st a1) <=? (aeval st a2)
  | ~ b => negb (beval_short st b)
  | b1 && b2 => if beval_short st b1 then beval_short st b2 else false
  end.
Close Scope imp_scope.
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_WhileTrueCont : forall st st' st'' b c,
      beval st b = true ->
      st =[ c ]=> st' / SContinue ->
      st' =[ WHILE b DO c END ]=> st'' / SContinue ->
      st =[ WHILE b DO c END ]=> st'' / SContinue
...
Proof.
  intros.
  inversion H0; subst; try (apply E_WhileTrueBreak; assumption).
Qed.
(** [] *)
...
Proof.
  ...
  induction H; try discriminate; inversion Heqloop; subst.
...
Proof.
  ...
  induction E1; intros; inversion E2; subst; try (split; reflexivity); try (apply IHE1; assumption);
    try (rewrite H in H6); try (rewrite H in H2); try rewrite H in H5; try discriminate.
  - (* ;; *)
    apply IHE1_2. destruct (IHE1_1 _ _ H1). subst. assumption.
  - destruct (IHE1_1 _ _ H4). discriminate.
  - destruct (IHE1 _  _ H1). discriminate.
  - (* while *)
    apply IHE1_2. destruct (IHE1_1 _ _ H3). subst. assumption.
  - destruct (IHE1_1 _ _ H6). discriminate.
  - destruct (IHE1 _ _ H3). discriminate.
  - destruct (IHE1 _ _ H6). subst. split; reflexivity.
Qed.  
...
(* 请在此处解答
    ...
    [] *)
Notation "'FOR' c1 'COND' b 'STEP' c3 'DO' c2 'END'" := (c1;; (WHILE b DO c2;; c3 END))%imp
