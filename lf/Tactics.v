...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o).
Proof.
  intros n1 m1 o1 p1 eq1 eq2.
  apply trans_eq with m1.
  apply eq2. apply eq1.
Qed.
...
Proof.
  ...
  assert (eq5: y :: l = z :: l). { rewrite eq4. apply eq2. }
  injection eq5 as eq6.
  rewrite eq6. apply eq3.
Qed.
(** [] *)
...
(** 我们来看一下此定理的非形式化证明。注意我们保持 [n]
    ...
    _'证明'_：令 [m] 为一个 [nat]。我们通过对 [m] 进行归纳来证明，对于任何 [n]，
      ...
      - 其次，设 [m = S m']，而 [n] 同样是一个数使得 [double n = double m]。
        ...
        故存在 [n'] 使得 [n = S n']。再次根据 [double] 之定义，可得 [S (S
        (double n')) = S (S (double m'))]。再由构造子单射可知 [double n' =
        double m']。以[n'] 代入归纳假设，推得 [n' = m']，故显然 [S n' = S m']，
        其中 [S n' = n]，[S m' = m]，所以原命题得证。 [] *)
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  Proof.
  intros X Y l.
  induction l as [| (x, y) l' IHl']; intros l1 l2 H.
  - injection H as H1 H2. rewrite <- H1. rewrite <- H2. reflexivity.
  - simpl in H.
    ...
    + discriminate H1.
    + discriminate H2.
    + injection H1 as Hx Hlx.
      injection H2 as Hy Hly.
      rewrite Hx. rewrite Hy.
      simpl. f_equal.
      apply IHl'.
      f_equal. apply Hlx. apply Hly.
Qed.
(** [] *)
...
Proof.
  ...
  assert (forall n: nat, (0 =? n) = (n =? 0)) as le1.
  { destruct n.
    - reflexivity.
    - rewrite zero_nbeq_S. symmetry. apply S_nbeq_0. }
  intros n.
  ...
  - intros m.
    ...
    + symmetry. apply le1.
    + simpl. apply IHn'.
  (* 用 eqb_true: forall n m: nat (n =? m) = true -> n = m
     和 eqb_refl: forall n : nat, true = (n =? n) 的另一种证明
  intros n m.
  destruct (n =? m) eqn:E1.
  - apply eqb_true in E1.
    rewrite E1. apply eqb_refl.
  - destruct (m =? n) eqn:E2.
    + apply eqb_true in E2.
      rewrite E2 in E1.
      rewrite <- E1. symmetry. apply eqb_refl.
    + reflexivity.
  *)
Qed.
(** [] *)
...
Proof.
  assert (forall (X:Type) (l:list X), 0 = length l -> [] = l) as length_0.
  { destruct l.
    - reflexivity.
    - intros H. simpl in H. discriminate H.
  }
  intros X l1.
  induction l1 as [| a l1']; intros l2 H.
  - simpl. f_equal.
    simpl in H. apply length_0 in H. apply H.
  - destruct l2 as [| b l2'].
    + simpl in H. discriminate H.
    + simpl. destruct (split (combine l1' l2')) eqn:E.
      simpl in H. injection H as H. apply IHl1' in H. rewrite E in H.
      injection H as H1 H2.
      f_equal; f_equal.
      apply H1. apply H2.
Qed.
...
Proof.
  ...
  induction l as [| a l' IHl']; intros x lf H.
  - simpl in H. discriminate H.
  - simpl in H.
    destruct (test a) eqn:E.
    + injection H as H1.
      rewrite H1 in E. apply E.
