...
Definition peven : com :=
  ...
  WHILE ~(X = 0) DO
    ...
    X ::= X - 1
  END.
Example peven_0: test_ceval (X !-> 0) peven = Some(0, 0, 0).
Proof. reflexivity . Qed.
Example peven_1: test_ceval (X !-> 1) peven = Some(0, 0, 1).
Proof. reflexivity . Qed.
Example peven_2: test_ceval (X !-> 2) peven = Some(0, 0, 0).
Proof. reflexivity . Qed.
...
Proof.
  ...
  - (* ;; *)
    ...
    destruct (Nat.le_ge_cases i1 i2).
    + exists (S i2).
      simpl. assert (ceval_step st c1 i2 = Some st') as H3.
      apply ceval_step_more with i1; assumption.
      rewrite H3. assumption.
    + exists (S i1). simpl. rewrite H1. apply ceval_step_more with i2; assumption.
  - (* TEST true *)
