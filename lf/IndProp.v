...
Theorem ev5_nonsense :
  ev 5 -> 2 + 2 = 9.
Proof.
  intros H. inversion H as [|n [|n' []] EQ].
Qed.
...
Proof.
  split.
  - intros Hn. induction Hn as [| | n' E IH].
    + apply ev_0. + apply (ev_SS 0). apply ev_0.
    + apply ev_sum. apply IHIH. apply IHHn1.
  - intros Hn. induction Hn as [|n' E IH].
    + apply ev'_0.
...
Proof.
  intros h m p Hnm Hnp.
  apply ev_ev__ev with (h + p).
  - (* 无聊的改写 *)
    rewrite plus_assoc.
...
Inductive total_relation: nat -> nat -> Prop :=
  | ttr a b : total_relation a b.

Theorem total_relation_total: forall a b:nat, total_relation a b.
Proof. intros a b. apply ttr. Qed.

(** **** 练习：2 星, standard, optional (empty_relation) 

    请定一个二元归纳关系 [empty_relation] 对自然数永远为假。 *)
Inductive empty_relation: nat -> nat -> Prop := .
Theorem empty_relation_empty: forall a b:nat, ~ empty_relation a b.
Proof. intros a b H. inversion H. Qed.
...
Proof.
  ...
  generalize dependent m.
  induction o.
  - intros m n Hmn Hno. inversion Hno. rewrite H in Hmn. apply Hmn.
  - intros m n Hmn HnSo.
    inversion HnSo.
    + rewrite H in Hmn. apply Hmn.
    + apply (le_S _ _ (IHo _ _ Hmn H0)).
Qed.

Theorem O_le_n : forall n,
  0 <= n.
Proof.
  intros n. induction n as [|n' IHn']. apply le_n.
  apply (le_S _ _ IHn').
Qed.
...
Proof.
  ...
  - intros n Hn. inversion Hn. reflexivity.
  - intros n Hnm'. inversion Hnm'.
    + reflexivity.
    + apply (le_S _ _ (IHm' _ H0)).
Qed.
...
Proof.
  ...
  inversion HSnm.
  - reflexivity.
  - apply (le_trans _ _ _ (le_S _ _ (le_n n)) H0).
Qed.
...
Proof.
  split.
  - apply le_trans with (n:=n1+n2). apply le_plus_l. apply H.
  - rewrite plus_comm in H.
    apply le_trans with (n:=n2+n1). apply le_plus_l. apply H.
Qed.
...
Proof.
  ...
  - left. apply O_le_n.
  - destruct p.
    + right. destruct (plus_le (S n) m q H) as [_ H1].
      apply H1.
    + simpl. intros q H. apply Sn_le_Sm__n_le_m in H.
      destruct (IHn m p q H) as [H1 | H1].
...
Proof.
  intros n1 n2 m H.
  unfold lt in H.
  destruct (plus_le (S n1) _ _ H) as [H1 _].
  rewrite plus_n_Sm in H.
  destruct (plus_le _ _ _ H) as [_ H2].
...
Proof.
  ...
  - intros m Hm. destruct m.
    ...
    + simpl in Hm. apply n_le_m__Sn_le_Sm. apply IHn. apply Hm.
Qed.
...
Proof.
  ...
  apply (leb_correct _ _ (le_trans _ _ _ Hnm Hmo)).
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. induction H.
    ...
    + simpl in IHR. rewrite <- plus_n_Sm in IHR. injection IHR as IH.
      apply IH.
    + rewrite plus_comm. apply IHR.
  - generalize dependent o. induction m.
    + intros o H. simpl in H. rewrite H. generalize o. induction o0.
      * apply c1.
      * apply c3. apply IHo0.
    + intros o H. destruct o.
      * discriminate H.
      * injection H as H. apply (c2 _ _ _ (IHm _ H)).
Qed.
(** [] *)
...
Inductive subseq : list nat -> list nat -> Prop :=
  | subseq_empty : subseq [] []
  | subseq_A x l1 l2 (H: subseq l1 l2): subseq (x :: l1) (x :: l2)
  | subseq_R x l1 l2 (H: subseq l1 l2): subseq l1 (x :: l2)
.
...
Proof.
  ...
  - inversion H. induction l3.
    + apply subseq_empty.
    + apply subseq_R. apply IHl3.
  - inversion H.
    + simpl. apply subseq_A. apply IHl2. apply H2.
    + simpl. apply subseq_R. apply IHl2. apply H2.
Qed.
...
Proof.
  ...
  induction l3.
  - intros l1 l2 H H1.
    inversion H1. rewrite <- H0 in H.
    inversion H. apply subseq_empty.
  - intros l1 l2 H H1.
    inversion H1 as [|x0 l2' l3' Hmatch|].
    + rewrite <- H0 in H. inversion H.
      * rewrite <- H2. apply subseq_A. apply IHl3 with (l2:=l2').
        apply H6. apply Hmatch.
      * apply subseq_R. apply IHl3 with l2'. apply H6. apply Hmatch.
    + apply subseq_R. apply (IHl3 l1 l2 H H0).
Qed.
(** [] *)
...
Proof.
  ...
  intros T ss re H.
  induction ss as [|x ss' IHss'].
  - simpl. apply MStar0.
  - simpl. apply MStarApp.
    + apply H. simpl. left. reflexivity.
    + apply IHss'. intros s Hs. apply H.
...
Proof.
  ...
  - intros H.
    ...
    induction s1 as [|x s1' IHs1'].
    + intros s2 H.
      destruct s2. { reflexivity. }
      simpl in H. inversion H as [| |[] | | | |].
      inversion H1.
    + intros s2 H. destruct s2.
      * simpl in H. inversion H.
      * simpl in H. inversion H.
        inversion H3.
        simpl. f_equal.
        rewrite <- H5 in H1. inversion H1.
        apply IHs1'. rewrite <- H9. apply H4.
  - intros H. rewrite H.
    generalize s2 as s0.
    induction s0 as [|x s' IHs'].
    + apply MEmpty.
    + simpl. apply (MApp [x]).
      * apply MChar.
      * apply IHs'.
...
Proof.
  ...
  - intros H.
    ...
    + simpl in H. apply andb_true_iff in H.
      destruct H as [H1 H2].
      destruct (IHr1 H1) as [s1 H3].
      destruct (IHr2 H2) as [s2 H4].
      exists (s1 ++ s2).
      apply MApp. apply H3. apply H4.
    + simpl in H. apply orb_true_iff in H.
      destruct H as [H|H].
      * destruct (IHr1 H) as [s Hs]. exists s. apply MUnionL. apply Hs.
      * destruct (IHr2 H) as [s Hs]. exists s. apply MUnionR. apply Hs.
    + exists []. apply MStar0.
Qed.
(** [] *)
...
Proof.
  intros T s re Hs.
  remember (Star re) as re'.
  induction Hs as [| | | | |re0|s1 s2 re0 Hmatch1 IH1 Hmatch2 IH2].
  - discriminate.
  - discriminate.
  - discriminate.
  - discriminate.
  ...
  - exists []. split.
    * reflexivity.
    * intros s' [].
  - destruct (IH2 Heqre') as [s0 Hs0].
    exists (s1 :: s0). destruct Hs0 as [Hs1 Hs2]. split.
    + simpl. f_equal. apply Hs1.
    + simpl. intros s' Hs'. destruct Hs' as [Hs' | Hs'].
      * injection Heqre' as H. rewrite <- H. rewrite <- Hs'. apply Hmatch1.
      * apply (Hs2 _ Hs').
Qed.
(** [] *)
...
Theorem app_assoc4: forall T (l1 l2 l3 l4 : list T), (l1 ++ l2 ++ l3) ++ l4 = l1 ++ l2 ++ l3 ++ l4.
Proof. intros T l1 l2 l3 l4. rewrite <- app_assoc. replace (l2 ++ l3 ++ l4) with ((l2 ++ l3) ++ l4).
  reflexivity. symmetry. apply app_assoc.
Qed.
...
Proof.
  ...
  - simpl. intro H. inversion H. inversion H1.
  - simpl. intros H. rewrite app_length in H. apply add_le_cases in H. destruct H as [H | H].
    + apply IH1 in H. destruct H as [s2' [s3' [s4']]].
      destruct H as [H1 [H2 H3]].
      exists s2'. exists s3'. exists (s4' ++ s2).
      split. rewrite H1. apply app_assoc4.
      split. apply H2.
      intros m. replace (s2' ++ napp m s3' ++ s4' ++ s2) with ((s2' ++ napp m s3' ++ s4') ++ s2).
      apply MApp. apply H3. apply Hmatch2. apply app_assoc4.
    + apply IH2 in H. destruct H as [s1' [s3' [s4' [H1 [H2 H3]]]]].
      exists (s1 ++ s1'). exists s3'. exists s4'.
      rewrite H1. split. rewrite app_assoc. reflexivity.
      split. apply H2. intros m. rewrite <- app_assoc. apply MApp. apply Hmatch1. apply H3.
  - simpl. intros H. destruct (plus_le _ _ _ H) as [H1 _].
    destruct (IH H1) as [s2 [s3 [s4 [H2 [H3 H4]]]]].
    exists s2. exists s3. exists s4. split. apply H2. split. apply H3. intros m.
    apply MUnionL. apply H4.
  - simpl. intros H. destruct (plus_le _ _ _ H) as [_ H1].
    destruct (IH H1) as [s1 [s3 [s4 [H2 [H3 H4]]]]].
    exists s1. exists s3. exists s4. split. apply H2. split. apply H3. intros m.
    apply MUnionR. apply H4.
  - simpl. intros H. inversion H. apply pumping_constant_0_false in H1. destruct H1.
  - simpl. rewrite app_length. intros H. destruct s1.
    + simpl in H. apply IH2 in H. destruct H as [s1' [s3' [s4' [H1 [H2 H3]]]]].
      exists s1'. exists s3'. exists s4'. split. apply H1. split.
      apply H2. apply H3.
    + exists []. exists (x :: s1). exists s2. split. reflexivity. split. discriminate.
      intros m. apply napp_star. apply Hmatch1. apply Hmatch2.
Qed.
(** [] *)
...
Theorem leb_false_le : forall a b:nat, (a <=? b) = false -> (b <= a).
Proof.
  induction a.
  - intros b H. discriminate H.
  - intros b H. destruct b.
    + apply O_le_n.
    + simpl in H. apply IHa in H. apply n_le_m__Sn_le_Sm. apply H.
Qed.

Theorem le_sum : forall a b c, (a <= b) -> (a + c <= b + c).
Proof.
  intros a b c H.
  generalize dependent a.
  generalize dependent b.
  induction c; intros b a H.
  - rewrite <- plus_comm. simpl. rewrite <- plus_comm. apply H.
  - rewrite <- plus_n_Sm. rewrite <- plus_n_Sm. apply n_le_m__Sn_le_Sm.
    apply IHc. apply H.
Qed.

Theorem le_sum4 : forall a b c d, (a <= b) -> (c <= d) -> (a + c <= b + d).
Proof.
  intros a b c d H1 H2.
  apply le_trans with (a + d).
  replace (a + d) with (d + a). rewrite plus_comm. apply (le_sum _ _ _ H2).
  apply plus_comm.
  apply (le_sum _ _ _ H1).
Qed.

Theorem le_complete : forall a b, a <= b \/ b <= a.
Proof.
  induction a.
  - left. apply O_le_n.
  - intros b.
...
Proof.
  ...
  - simpl. intros H. rewrite app_length in H. apply add_le_cases in H.
    (* 如果直接 destruct 会在第二个分支遇到需要 length s1 <= pumping_constant re1 才能推理下去的情形，
      是因为 destruct 出的 2 个分支 pc re1 <= s1 和 pc re2 <= se2，而我们想要 pc re1 <= 1 和 ~ (pr re1 <= 1) /\ pc re2 <= re2
      意味着在 destruct 前需要进一步推理，注意我们不可能证明 forall A B, A \/ B -> A \/ ~ A /\ B
      所以需要利用 <= 和 <=? 的等价性做处理，即
      forall A B, A = true \/ B = true -> A = true \/ A = false /\ B = true
      P.S. 其实可以对于特定的命题 f 证明 forall a b, f A \/ f b -> f A \/ ~ (f A) /\ f B *)
    rewrite <- leb_iff in H. rewrite <- leb_iff in H.
    remember (pumping_constant re1 <=? length s1) as leb.
    destruct leb.
    + symmetry in Heqleb. rewrite leb_iff in Heqleb. apply IH1 in Heqleb.
      destruct Heqleb as [s2' [s3' [s4' [H1 [H2 [H3 H4]]]]]].
      exists s2'. exists s3'. exists (s4' ++ s2).
      split. rewrite H1. apply app_assoc4. split. apply H2.
      split. apply le_trans with (pumping_constant re1). apply H3. apply le_plus_l.
      intros m. rewrite <- app_assoc4. apply MApp. apply H4. apply Hmatch2.
    + destruct H as [H | H]. discriminate H. symmetry in Heqleb. apply leb_false_le in Heqleb.
      rewrite leb_iff in H. apply IH2 in H. destruct H as [s1' [s3' [s4' [H1 [H2 [H3 H4]]]]]].
      exists (s1 ++ s1'). exists s3'. exists s4'. split. rewrite H1. apply app_assoc.
      split. apply H2. split. rewrite app_length.
      rewrite <- plus_assoc. apply (le_sum4 _ _ _ _ Heqleb H3).
      intros m. rewrite <- app_assoc. apply (MApp _ _ _ _ Hmatch1 (H4 _)).
  - simpl. intros H. apply plus_le in H.  destruct H as [Ha _].
    apply IH in Ha. destruct Ha as [s2' [s3' [s4' [H1 [H2 [H3 H4]]]]]].
    exists s2'. exists s3'. exists s4'. split. apply H1. split. apply H2.
    split. apply le_trans with (pumping_constant re1).
    apply H3. apply le_plus_l. intros m. apply MUnionL. apply H4.
  - simpl. intros H. apply plus_le in H.  destruct H as [_ Ha].
    apply IH in Ha. destruct Ha as [s2' [s3' [s4' [H1 [H2 [H3 H4]]]]]].
    exists s2'. exists s3'. exists s4'. split. apply H1. split. apply H2.
    split. apply le_trans with (pumping_constant re2).
    apply H3. rewrite plus_comm. apply le_plus_l. intros m. apply MUnionR. apply H4.
  - simpl. intros H. apply (le_trans _ _ _ (pumping_constant_ge_1 _ _)) in H.
    inversion H.
  - simpl. rewrite app_length. intros H.
    destruct (le_complete (pumping_constant re) (length s1)) as [H0 | H0].
    + apply IH1 in H0. destruct H0 as [s2' [s3' [s4' [H1 [H2 [H3 H4]]]]]].
      exists s2'. exists s3'. exists (s4' ++ s2). rewrite H1.
      split. apply app_assoc4. split. apply H2. split. apply H3. intros m.
      rewrite <- app_assoc4.
      replace (s2' ++ napp m s3' ++ s4') with (napp 1 (s2' ++ napp m s3' ++ s4')).
      apply napp_star. apply H4. apply Hmatch2. simpl. apply app_nil_r.
    + destruct s1.
      * apply IH2 in H. destruct H as [s1' [s3' [s4' [H1 [H2 [H3 H4]]]]]].
        exists s1'. exists s3'. exists s4'. split. rewrite H1. reflexivity.
        split. apply H2. split. apply H3. apply H4.
      * exists []. exists (x :: s1). exists s2. split. reflexivity.
        split. discriminate. split. apply H0. intros m. apply napp_star.
        apply Hmatch1. apply Hmatch2.
Qed.
(* 总结：这个命题证明的关键在与命题本身，pumping constant 提供的构造导致一个构造性的证明，允许在 coq 中出现。
   take away: 良好的内外命名，内部命名当命题变得复杂时更重要，外部的命名当命题依赖的引理多的时候重要；
   学习 Search 的高级用法也有帮助
   想法：coq 能否允许部分分支 admitted，先进行后面的分支
   vscoq 能否在 induction / destruct 建立分支时自动填充分支注释 *)
...
Proof.
  ...
  - split. intros _. reflexivity. intros _. apply H0.
  - split. intros HP. apply H0 in HP. destruct HP.
...
(* 之前我使用过的不够一般的技巧 *)
Theorem eq_or_neq : forall n m:nat, n = m \/ n <> m.
Proof.
  intros n m. rewrite <- eqb_eq.
  destruct (n =? m).
  (* iff_reflect 相当于帮你省下了下面的证明 *)
  - left. reflexivity.
  - right. discriminate.
Qed.
...
Proof.
  ...
  - simpl in H. destruct (eqbP n a) as [H0 | H0].
    + discriminate H.
    + simpl. intros H1. destruct H1 as [H1 | H1]. apply H0. rewrite H1. reflexivity.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  ...
  | nostutter_one a : nostutter [a]
  | nostutter_app a b l (H: nostutter (b :: l)) (Hneq: a <> b): nostutter (a :: b :: l)
.
...
(* inorder merge l1 l2 l3 : l3 is in-order merge of l1 l2 *)
Inductive inorder_merge {X: Type} : (list X) -> (list X) -> (list X) -> Prop :=
  | inorder_merge_nil : inorder_merge [] [] []
  | inorder_merge_l a l1 l2 l3 (H: inorder_merge l1 l2 l3): inorder_merge (a :: l1) l2 (a :: l3)
  | inorder_merge_r a l1 l2 l3 (H: inorder_merge l1 l2 l3): inorder_merge l1 (a :: l2) (a :: l3)
.

Theorem filter_merge : forall (X:Type) (test: X ->bool) (l l1 l2 : list X),
  inorder_merge l1 l2 l -> All (fun x => test x = true) l1 ->
  All (fun x => test x = false) l2 -> filter test l = l1.
Proof.
  intros X test l l1 l2 H1 H2 H3.
  induction H1 as [| a l1' l2' l3' Hmerge IH | a l1' l2' l3' Hmerge IH].
  - reflexivity.
  - simpl in H2. destruct H2 as [Ha H2]. apply IH in H2.
    simpl. rewrite Ha. f_equal. apply H2. apply H3.
  - simpl in H3. destruct H3 as [Ha H3]. apply IH in H3.
    simpl. rewrite Ha. f_equal. apply H3. apply H2.
Qed.
...
Theorem test_filter : forall (X : Type) (test : X -> bool) (l : list X),
  All (fun x => test x = true) (filter test l).
Proof.
  intros X test l.
  induction l as [| a l' IHl'].
  - simpl. apply I.
  - simpl. destruct (test a) eqn:E.
    + simpl. split. apply E. apply IHl'.
    + apply IHl'.
Qed.

(* 为了使用 subseq (nat) 下面的命题定义在 list nat 上 *)

Theorem subseq_le : forall l1 l2, subseq l1 l2 -> length l1 <= length l2.
Proof.
  intros l1 l2. generalize dependent l1.
  induction l2 as [| a l2' IHl2'].
  - intros l1 H. inversion H. reflexivity.
  - intros l1 H. inversion H.
    + simpl. apply n_le_m__Sn_le_Sm. apply IHl2'. apply H2.
    + simpl. apply le_S. apply IHl2'. apply H2.
Qed.

Theorem filter_longest_nat : forall test l lo,
  subseq lo l -> All (fun x => test x = true) lo -> length lo <= length (filter test l).
Proof.
  intros test l lo H1 H2.
  apply subseq_le.
  generalize dependent lo.
  induction l as [| a l']; intros lo Hlo Htest.
  - simpl. apply Hlo.
  - simpl. inversion Hlo.
    + (*lo = x :: l1 *) rewrite <- H0 in Htest. simpl in Htest.
      destruct Htest as [Htest HAll].
      rewrite H in Htest. rewrite Htest. apply subseq_A. apply IHl'.
      apply H1. apply HAll.
    + apply IHl' in H1. destruct (test a). apply subseq_R. apply H1. apply H1. apply Htest.
Qed.

(* 提示：用 merge 刻画 subseq，给出任意类型的定理 *)

Theorem filter_longest : forall X test l l1 l2,
  inorder_merge l1 l2 l -> All (fun x => test x = true) l1 ->
  length l1 <= @length X (filter test l).
Proof.
  intros X test l l1 l2 H.
  induction H as [| a l1' l2' l3' H IH | a l1' l2' l3' H IH].
  - intros _. simpl. apply le_n.
  - intros [Htest HA]. simpl. rewrite Htest. simpl.
    apply n_le_m__Sn_le_Sm. apply IH. apply HA.
  - intros HA. apply IH in HA. simpl. destruct (test a).
    + simpl. apply le_S. apply HA.
    + apply HA.
Qed.
...
Inductive pal {X : Type} : list X -> Prop :=
  | pal_nil : pal []
  | pal_x x : pal [x]
  | pal_a (a : X) (x : list X) (H: pal x) : pal (a :: x ++ [a])
.
...
Proof.
  ...
  induction l as [| a l'].
  - simpl. apply pal_nil.
  - simpl. apply pal_a with (a:=a) in IHl'. rewrite app_assoc.
    apply IHl'.
Qed.
...
Proof.
  intros X l H.
  induction H as [| x | a x' H].
  - reflexivity.
  - reflexivity.
  - replace (a :: x' ++ [a]) with ((a :: x') ++ [a]).
    rewrite rev_app_distr. simpl. rewrite <- IHH. reflexivity.
    reflexivity.
Qed.
...
Theorem ev_ev1 : forall n, ev n \/ ev (1 + n).
Proof.
  induction n.
  - left. apply ev_0.
  - destruct IHn as [H | H].
    * right. apply ev_SS. apply H.
    * left. apply H.
Qed.
...
Proof.
  intros X l.
  destruct (ev_ev1 (length l)).
  * remember (length l) as len.
    generalize dependent l.
    induction H.
    - intros l H0 _. destruct l. apply pal_nil. simpl in H0. discriminate.
    - intros l H0 H1. destruct l. apply pal_nil.
      (* x::l = rev (x::l) *)
      simpl in H1. destruct (rev l) eqn:E.
      + simpl in H1. rewrite H1 in H0. simpl in H0. discriminate.
      + simpl in H1. injection H1 as H2. rewrite H1. apply pal_a. apply IHev.
        rewrite H1 in H0. simpl in H0. rewrite app_length in H0.
        rewrite plus_comm in H0. simpl in H0. injection H0 as H0. apply H0.
        rewrite H1 in E. rewrite rev_app_distr in E. simpl in E. injection E as H3 H4.
        rewrite H4. reflexivity.
  * remember (1 + length l) as len.
    generalize dependent l.
    induction H.
    - discriminate.
    - intros l H0 H1. destruct l. apply pal_nil.
      simpl in H1. destruct (rev l) eqn:E.
      + simpl in H1. rewrite H1. apply pal_x.
      + (* copy *)
        simpl in H1. injection H1 as H2. rewrite H1. apply pal_a. apply IHev.
        rewrite H1 in H0. simpl in H0. rewrite app_length in H0.
        rewrite plus_comm in H0. simpl in H0. injection H0 as H0. apply H0.
...
(* 失败的尝试：缺乏证据，加强归纳假设，打包证明 *)
Theorem rev_pal' : forall X (l : list X) a, (l = rev l -> pal l) /\ (l ++ [a] = rev (l ++ [a]) -> pal (l ++ [a])).
Proof.
  intros X. induction l.
  - simpl. intros a. split; intros _. apply pal_nil. apply pal_x.
  - simpl. intros a. split; intros H.
    + destruct (rev l) eqn:E.
      * injection H as H1. rewrite H1. apply pal_x.
...
Definition disjoint {X : Type} : list X -> list X -> Prop :=
  fun (l1 l2 : list X) => forall x, In x l1 -> In x l2 -> False.
...
Inductive NoDup {X : Type} : list X -> Prop :=
  | nodup_nil : @NoDup X []
  | nodup_app x l (H1: NoDup l) (H2: ~ In x l) : NoDup (x :: l).
...
Proof.
  induction l1.
  - intros l2 _ H2 _. apply H2.
  - simpl. intros l2 H1 H2 H3. apply nodup_app. apply IHl1.
    inversion H1. apply H5.
    apply H2.
    unfold disjoint in H3. intros x0 H. apply H3. simpl. right. apply H.
    intros contra.
    assert (forall l : list X, In x (l ++ l2) -> In x l \/ In x l2). {
      induction l.
      simpl. intros H. right. apply H.
      simpl. intros [H | H]. left. left. apply H.
      apply IHl in H. destruct H. left. right. apply H. right. apply H.
    }
    apply H in contra.
    destruct contra.
    + inversion H1. apply H7. apply H0.
    + assert (Hxl1: In x (x :: l1)). left. reflexivity.
      unfold disjoint in H3. apply H3 with x. apply Hxl1. apply H0.
Qed.
...
Proof.
  induction l.
  - intros contra. simpl in contra. destruct contra.
  - intros [H | H].
    + exists []. exists l. simpl. rewrite H. reflexivity.
    + apply IHl in H. destruct H as [la [lb]].
      exists (x0 :: la). exists lb. simpl. rewrite H. reflexivity.
Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
  | repeat_app l1 a l2 (H: In a l2) : repeats (l1 ++ a :: l2)
.
...
Theorem in_app_or : forall X (l1 l2:list X) (x:X),
  In x (l1 ++ l2) <-> In x l1 \/ In x l2.
Proof.
  split.
  - generalize dependent l2.
    induction l1.
    + simpl. intros l2 H. right. apply H.
    + simpl. intros l2 [H | H].
      * left. left. apply H.
  ...
  - generalize dependent l2.
    induction l1.
    + simpl. intros l2 [[] | H]. apply H.
    + simpl. intros l2 [[H|H]|H].
      * left. apply H.
...
Proof.
  ...
  - simpl. intros l2 _ _ contra. inversion contra.
  - intros l2 ex H H1.
    destruct (ex (In x l1')) as [H0 | H0].
    + apply (repeat_app [] x l1' H0).
    + assert (H2: In x (x :: l1')). {
        simpl. left. reflexivity.
      }
      destruct (in_split _ _ _ (H x H2)) as [l2l [l2r]].
      assert (repeats l1'). {
        apply IHl1' with (l2l ++ l2r). { apply ex. }
        - intros x0 Hx0. assert (Hx0': In x0 (x :: l1')).
          { simpl. right. apply Hx0. }
          apply H in Hx0'. rewrite H3 in Hx0'. apply in_app_or in Hx0'.
          destruct Hx0' as [H4 | H4]. apply in_app_or. left. apply H4.
          apply in_app_or. right. simpl in H4. destruct H4 as [H4 | H4].
          rewrite <- H4 in Hx0. destruct (H0 Hx0). apply H4.
        - (*length*) apply le_trans with (length l2).
          rewrite H3. rewrite app_length. rewrite app_length. simpl.
          rewrite <- plus_n_Sm. apply le_n.
          simpl in H1. apply Sn_le_Sm__n_le_m. apply H1.
      }
      (* repeats l1 *)
      inversion H4.
      apply (repeat_app (x :: l1) a l0). apply H5.
Qed.
...
Proof.
  ...
  - intros. inversion H. destruct s1.
    ...
    + injection H1 as H1. right. exists s1, s2.
      split. symmetry. apply H5. split. rewrite <- H1. apply H3. apply H4.
  - intros [[H1 H2] | [s0 [s1 [H1 [H2 H3]]]]].
    + apply (MApp [] _ (a :: s) _ H1 H2).
    + rewrite H1. apply (MApp (a :: s0) _ (s1) _ H2 H3).
Qed.
(** [] *)
...
Proof.
  split.
  - intros.
    remember (a :: s) as l. remember (Star re) as re_.
    ...
    + discriminate.
    + destruct s1.
      * destruct (IHexp_match2 Heql Heqre_) as [s0 [s1 H1]].
        exists s0, s1. apply H1.
      * injection Heql as Hx Hs.
        exists s1, s2. split. rewrite Hs. reflexivity. split.
        rewrite <- Hx.  injection Heqre_ as Hre. rewrite <- Hre. apply H.
        apply H0.
  - intros [s0 [s1 [H1 [H2 H3]]]].
    rewrite H1. apply (MStarApp (a :: s0) s1 _ H2 H3).
Qed.
(** [] *)
...
Proof.
  ...
  - apply ReflectT. induction re.
    ...
    + apply MEmpty.
    + discriminate.
    + simpl in E. apply andb_true_iff in E. destruct E as [H1 H2].
      apply IHre1 in H1. apply IHre2 in H2.
      apply MApp with (s1:=[]). apply H1. apply H2.
    + simpl in E. apply orb_true_iff in E. destruct E as [H | H].
      * apply IHre1 in H. apply MUnionL. apply H.
      * apply IHre2 in H. apply MUnionR. apply H.
    + apply MStar0.
  - apply ReflectF. induction re.
    + apply quiz.
    + discriminate.
    + intros H. inversion H.
    + intros H. simpl in E. destruct (match_eps re1) eqn:Ere1.
      * simpl in E. apply IHre2 in E. apply E. inversion H.
        destruct s1,s2. apply H4. discriminate. discriminate. discriminate.
      * assert (false = false) as H1. reflexivity. apply IHre1 in H1. apply H1.
        inversion H. destruct s1, s2. apply H4. discriminate. discriminate. discriminate.
    + intros H. simpl in E. destruct (match_eps re1).
      * discriminate.
      * assert (false = false) as H1. reflexivity. apply IHre1 in H1. simpl in E.
        apply IHre2 in E. inversion H. destruct (H1 H3). destruct (E H3).
    + discriminate.
Qed.
(** [] *)
...
Fixpoint derive (a : ascii) (re : reg_exp ascii) : reg_exp ascii :=
  ...
  | EmptySet => EmptySet
  | EmptyStr => EmptySet
  | Char t => if eqb a t then EmptyStr else EmptySet (* eqb 在 ascii 中定义，证明中会用到 eqb_refl 和 eqb_eq/eqb_neq *)
  | App r1 r2 =>
    if match_eps r1 then
      Union (derive a r2) (App (derive a r1) r2)
    else
      App (derive a r1) r2
...
Proof.
  intros a re s. split; generalize dependent a; generalize dependent s; induction re; intros s a H.
  - inversion H.
  - inversion H.
  - simpl. inversion H. rewrite eqb_refl. apply MEmpty.
  - simpl. destruct (match_eps_refl re1) as [Hre1 | Hre1].
    + apply app_ne in H. destruct H as [ [_ Hre2] | [s0 [s1 [Hs [Hre1' Hre2]]]]].
      { apply MUnionL. apply IHre2. apply Hre2. }
      { apply MUnionR. rewrite Hs. apply MApp. apply IHre1. apply Hre1'.
        apply Hre2. }
    + apply app_ne in H. destruct H as [ [contra _] | [s0 [s1 [Hs [Hre1' Hre2]]]]].
      destruct (Hre1 contra).
      rewrite Hs. apply MApp. apply IHre1. apply Hre1'. apply Hre2.
  - inversion H; simpl.
    + apply MUnionL. apply IHre1. apply H2.
    + apply MUnionR. apply IHre2. apply H1.
  - apply star_ne in H. destruct H as [s0 [s1 [Hs [Hre H]]]].
    simpl. rewrite Hs. apply MApp. apply IHre. apply Hre. apply H.

  - inversion H.
  - inversion H.
  - simpl in H. destruct (a =? t)%char eqn:E.
    + inversion H. apply eqb_eq in E. rewrite E. apply MChar.
    + inversion H.
  - simpl in H. destruct (match_eps_refl re1) as [Hre1 | Hre1].
    + inversion H.
      * apply MApp with (s1:=[]). apply Hre1. apply IHre2. apply H2.
      * inversion H1. apply MApp with (s1:=a::s1). apply IHre1. apply H7.
        apply H8.
    + inversion H.
      * apply MApp with (s1:=a::s1). apply IHre1. apply H3. apply H4.
  - simpl in H. inversion H.
    + apply MUnionL. apply IHre1. apply H2.
    + apply MUnionR. apply IHre2. apply H1.
  - simpl in H. inversion H. apply MStarApp with (s1:=a::s1).
...
Proof.
  intros s re.
  remember (regex_match s re) as Hm.
  destruct Hm.
  - apply ReflectT. generalize dependent re.
    induction s; intros re H.
    + simpl in H. destruct (match_eps_refl re). apply H0. discriminate.
    + simpl in H. apply derive_corr. apply IHs. apply H.
  - apply ReflectF. generalize dependent re.
    induction s; intros re H.
    + simpl in H. destruct (match_eps_refl re). discriminate. apply H0.
    + simpl in H. intros H0. apply derive_corr in H0. apply (IHs (derive x re) H).
      apply H0.
Qed.
(** [] *)
