...
Proof.
  ...
  - destruct re1;
    try (destruct re2;
    try apply MApp; try apply IH1; try apply IH2; inversion IH2;
    rewrite app_nil_r; apply IH1).
    inversion IH1. destruct re2; apply IH2.
  - destruct re1; try (destruct re2; try apply IH; apply MUnionL; apply IH).
    inversion IH.
  - destruct re1; try apply IH;
    destruct re2; try (apply MUnionR; apply IH); inversion IH.
  - destruct re; try apply MEmpty; try apply MStar0.
  - destruct re; try apply star_app; try (apply MStar1; apply IH1); try apply IH2;
    inversion IH1. inversion IH2. apply MEmpty.
Qed.
...
(* antecedent and assertion *)
Definition ant {A B : Prop} (p : A -> B) : Prop := A.

Lemma app_assoc1 {T:Type} (a b c d:list T) : (a ++ b ++ c) ++ d = a ++ b ++ c ++ d.
Proof.
  repeat (rewrite <- app_assoc; f_equal).
Qed.

Lemma app_assoc2 {T:Type} (a b c d:list T) : (a ++ b) ++ c ++ d = a ++ b ++ c ++ d.
Proof.
  repeat (rewrite <- app_assoc; f_equal).
Qed.

Hint Resolve MApp.
Hint Resolve MUnionL.
Hint Resolve MUnionR.
Hint Resolve MStarApp.
Hint Resolve MStar0.

Ltac cons := constructor; auto.
Ltac li := simpl in *; lia.
Ltac spli := repeat split; try li; auto.
Ltac de IH := destruct IH as [a [b [c [E1 [E2 E3]]]]].
Ltac de2 IH := destruct IH as [a [b [c [E1 [E2 [E3 E4]]]]]].
...
Proof.
  intros.
  induction H; simpl in H0; try lia; try rewrite app_length in H0;
  try (assert (ant IHexp_match1 \/ ant IHexp_match2); unfold ant; try lia; destruct H2).
  - de IHexp_match1. auto.
    exists a, b, (c ++ s2).
    spli; intros; subst; try rewrite <- app_assoc1; auto.
  - de IHexp_match2. auto.
    exists (s1 ++ a), b, c.
    spli; intros; subst; try rewrite app_assoc2; auto.
  - de IHexp_match. lia.
    exists a, b, c. spli.
  - de IHexp_match. lia.
    exists a, b, c. spli.
  - pose proof (pumping_constant_ge_1 T re) as H1. lia.
  - destruct s1 as [| x s1'] eqn:Es1.
    + de IHexp_match2. li.
      exists a, b, c. spli.
    + exists [], (x :: s1'), s2. spli. discriminate.
      intros. simpl. replace (x :: s1') with s1 in *.
      induction m.
      * auto.
...
Proof.
  intros.
  induction H; simpl in H0; try lia; try rewrite app_length in H0.
  try (assert (ant IHexp_match1 \/ (~ant IHexp_match1 /\ ant IHexp_match2));
       unfold ant in *; try lia; destruct H2 as [H2 | [H2 H3]]).
  - de2 IHexp_match1. auto.
    exists a, b, (c ++ s2).
    spli; subst; intros; try rewrite <- app_assoc1; simpl; auto.
  - de2 IHexp_match2. auto.
    exists (s1 ++ a), b, c.
    spli; subst; intros; try rewrite app_assoc2; simpl; auto.
    rewrite app_length. lia.
  - de2 IHexp_match. lia. exists a,b,c. spli.
  - de2 IHexp_match. lia. exists a,b,c. spli.
  - pose proof (pumping_constant_ge_1 _ re) as contra. lia.
  - assert (ant IHexp_match1 \/ ~ant IHexp_match1); unfold ant in *. lia.
    destruct H2.
    + de2 IHexp_match1. auto.
      exists a, b, (c ++ s2). spli. rewrite E1. apply app_assoc1.
      intros.  rewrite <- app_assoc1. auto.
    + destruct s1 as [| x s1'] eqn:Es1.
      * de2 IHexp_match2. auto.
        exists a,b,c. spli.
      * exists [], (x :: s1'), s2. spli. discriminate.
        intros. simpl. replace (x :: s1') with s1 in *.
        induction m. auto. simpl. rewrite <- app_assoc. auto.
Qed.
(* 请勿修改下面这一行： *)
