...
Lemma mult_1_plus_r : forall n m : nat,
  n * S m = n + n * m.
Proof.
  intros n m.
  induction n.
...
Proof.
  ...
  - simpl. rewrite -> mult_1_plus_r. rewrite -> IHm. reflexivity.
Qed.
(** [] *)
...
(* induction on n is much more simple *)
...
Proof.
  intros n m.
  assert (Hn: forall p, n * p = p * n). { intros. rewrite mult_comm. reflexivity. }
  assert (Hm: forall p, m * p = p * m). { intros. rewrite mult_comm. reflexivity. }
  intros p.
  rewrite mult_comm.
  induction p.
  ...
  - simpl. rewrite -> IHp.
    ...
    assert (H: n + m + n * p = n + n * p + m).
    {
      rewrite <- plus_assoc.
      rewrite <- plus_assoc.
      assert (H': m + n * p = n * p + m). { rewrite plus_comm. reflexivity. }
      rewrite H'.
      reflexivity.
    }
    rewrite H.
...
Theorem bin_to_nat_pres_incr : forall b, S (bin_to_nat b) = bin_to_nat (incr b).
Proof.
  induction b.
  - reflexivity.
...
Proof.
  ...
  - reflexivity.
  - simpl.
    replace (S n) with (S (bin_to_nat (nat_to_bin n))).
...
Compute (normalize (A Z)).

Lemma nat_to_bin_B: forall n : nat,
nat_to_bin(1 + n + n) = B (nat_to_bin n).
Proof.
  induction n.
  - reflexivity.
  - replace (nat_to_bin (1 + S n + S n)) with (incr (incr (nat_to_bin(1 + n + n)))).
    + rewrite IHn.
      reflexivity.
    + rewrite <- plus_n_Sm.
      reflexivity.
...
Lemma nat_to_bin_A : forall n: nat,incr (nat_to_bin (n + S n)) = A (incr (nat_to_bin n)).
Proof.
  intros n.
  replace (n + S n) with (1 + n + n).
  rewrite nat_to_bin_B.
  reflexivity.
  rewrite <- plus_n_Sm.
  reflexivity.
Qed.

Theorem bin_nat_normbin : forall b : bin, nat_to_bin (bin_to_nat b) = normalize b.
Proof.
  induction b.
  - reflexivity.
  - simpl.
    ...
    + rewrite <- IHb.
      simpl.
      destruct (incr (nat_to_bin n)) eqn:H.
      * replace (S n) with O.
        rewrite <- plus_n_O.
        rewrite H.
        reflexivity.
        replace O with (bin_to_nat Z).
        rewrite <- H.
        rewrite <- bin_to_nat_pres_incr.
        rewrite -> nat_bin_nat.
        reflexivity.
