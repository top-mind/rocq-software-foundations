...
Check natlist1_ind :
  forall P : natlist1 -> Prop,
    P nnil1 -> (forall l, P l -> forall n, P (nsnoc1 l n)) ->
    forall l, P l.
...
Proof.
  ...
  - intros n IH m p. unfold P_pa in IH.
    simpl. f_equal. apply IH.
Qed.

Definition P_pc (n : nat) : Prop :=
  forall m : nat, n + m = m + n.

Theorem plus_comm3 : forall n : nat, P_pc n.
Proof.
  apply nat_ind.
  - intros m. apply plus_n_O.
  - intros. unfold P_pc in H. unfold P_pc. intros m.
...
Theorem evenb_ev_recall : forall n: nat, (evenb n = true -> ev n) /\ (evenb (S n) = true -> ev (S n)).
Proof.
  intros n.
  induction n; split; intros; try constructor; try (simpl in H; discriminate);
  destruct IHn as [IH1 IH2].
  apply IH2. assumption.
  apply IH1. simpl in H. assumption.
Qed.
